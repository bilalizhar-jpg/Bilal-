...
export default function AccountingSidebar({ isSidebarOpen }: AccountingSidebarProps) {
        ...
        <div className="space-y-1">
          <button
            ...
            className={`w-full flex items-center justify-between px-4 py-3.5 rounded-2xl transition-all duration-300 group ${
              isActive 
                ? isDark ? 'bg-[#2A2A3D] text-[#00FFCC]' : 'bg-indigo-50 text-indigo-700' 
                : isDark ? 'text-[#B0B0C3] hover:bg-[#2A2A3D] hover:text-white' : 'text-slate-600 hover:bg-slate-100 hover:text-slate-900'
            } ${depth === 1 ? 'pl-8' : depth === 2 ? 'pl-12' : ''}`}
          >
            <div className="flex items-center gap-4">
              {Icon && <Icon className={`w-5 h-5 transition-transform duration-300 group-hover:scale-110 ${isActive ? isDark ? 'text-[#00FFCC]' : 'text-indigo-600' : ''}`} />}
              {isSidebarOpen && (
          ...
          <AnimatePresence>
            {isOpen && (
              <motion.ul 
                ...
                className={`overflow-hidden space-y-1 ${depth === 0 ? 'ml-6 border-l ' + (isDark ? 'border-white/10' : 'border-slate-200') : ''}`}
              >
      ...
      <Link
        ...
        className={`flex items-center gap-4 px-4 py-3.5 rounded-2xl transition-all duration-300 group ${
          location.pathname === item.path
            ? isDark ? 'bg-[#2A2A3D] text-[#00FFCC] border border-[#00FFCC]/20' : 'bg-indigo-50 text-indigo-700' 
            : isDark ? 'text-[#B0B0C3] hover:bg-[#2A2A3D] hover:text-white' : 'text-slate-600 hover:bg-slate-100 hover:text-slate-900'
        } ${depth === 1 ? 'pl-10' : depth === 2 ? 'pl-14' : ''}`}
      >
        {Icon && <Icon className={`w-5 h-5 transition-transform duration-300 group-hover:scale-110 ${location.pathname === item.path ? isDark ? 'text-[#00FFCC]' : 'text-indigo-600' : ''}`} />}
  ...
  return (
    <aside className={`${isSidebarOpen ? 'w-72' : 'w-24'} relative z-30 flex flex-col transition-all duration-500 shrink-0 border-r print:hidden ${isDark ? 'bg-[#1B1B2F] border-white/5' : 'bg-white border-slate-200'}`}>
      <div className={`p-6 flex items-center gap-4 border-b h-20 ${isDark ? 'border-white/5' : 'border-slate-200'}`}>
        <div className={`p-0.5 rounded-xl shrink-0 overflow-hidden w-10 h-10 flex items-center justify-center shadow-lg ${isDark ? 'bg-gradient-to-tr from-[#00FFCC] to-[#00D1FF]' : 'bg-gradient-to-tr from-indigo-500 to-purple-500'}`}>
          <div className={`w-full h-full rounded-[10px] flex items-center justify-center overflow-hidden ${isDark ? 'bg-[#1E1E2F]' : 'bg-white'}`}>
            <Calculator className={`w-6 h-6 ${isDark ? 'text-[#00FFCC]' : 'text-indigo-600'}`} />
          </div>
        </div>
        {isSidebarOpen && (
          <span className={`font-display font-black text-xl tracking-tighter truncate uppercase ${isDark ? 'text-white' : 'text-slate-900'}`}>
            Accounting
          </span>
        )}
      </div>
      ...
      <nav className="flex-1 overflow-y-auto py-4 custom-scrollbar px-4">
        <div className={`px-4 mb-6 text-[10px] font-black uppercase tracking-[0.3em] ${isDark ? 'text-[#B0B0C3]' : 'text-slate-400'}`}>
      ...
      <div className={`p-6 border-t ${isDark ? 'border-white/5' : 'border-slate-200'}`}>
        <Link 
          to="/dashboard"
          className={`w-full flex items-center gap-4 px-4 py-3.5 rounded-lg transition-all duration-300 group mb-2 ${isDark ? 'text-[#B0B0C3] hover:bg-[#2A2A3D] hover:text-white' : 'text-slate-600 hover:bg-slate-100 hover:text-slate-900'}`}
        >
        ...
        <button 
          ...
          className={`w-full flex items-center gap-4 px-4 py-3.5 rounded-lg transition-all duration-300 group ${isDark ? 'text-[#B0B0C3] hover:bg-[#2A2A3D] hover:text-white' : 'text-slate-600 hover:bg-slate-100 hover:text-slate-900'}`}
        >
