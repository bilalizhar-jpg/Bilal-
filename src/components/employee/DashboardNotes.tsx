...
export function DashboardNotes({ notes, setNotes }: DashboardNotesProps) {
  ...
  return (
    <div className={`p-8 rounded-3xl border flex flex-col h-full ${isDark ? 'bg-slate-900/50 border-slate-800' : 'bg-white border-slate-100 shadow-xl shadow-slate-200/50'}`}>
      ...
      <div className="relative mb-6">
        ...
        <input
          ...
          className={`w-full pl-10 pr-4 py-3 rounded-xl text-sm font-medium outline-none transition-all ${
            isDark 
              ? 'bg-slate-800/50 text-white placeholder-slate-500 focus:bg-slate-800 focus:ring-2 focus:ring-indigo-500/50' 
          ...
          filteredNotes.map(note => (
            <div 
              ...
              className={`p-5 rounded-2xl border transition-all group ${
                note.isDone 
                  ? (isDark ? 'bg-slate-800/30 border-slate-800 opacity-60' : 'bg-slate-50 border-slate-100 opacity-60')
                  : (isDark ? 'bg-slate-800/80 border-slate-700 hover:border-indigo-500/50' : 'bg-white border-slate-200 hover:border-indigo-500/30 shadow-sm')
              }`}
            >
              <div className="flex items-start justify-between gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <h4 className={`text-sm font-bold truncate ${note.isDone ? 'line-through text-slate-500' : (isDark ? 'text-slate-200' : 'text-slate-800')}`}>
                    {note.title}
                  </h4>
                  {note.content && (
        ...
        {isModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
            <motion.div 
              ...
              className={`w-full max-w-md rounded-2xl shadow-xl overflow-hidden ${isDark ? 'bg-[#1E1E2F] border border-white/10' : 'bg-white'}`}
            >
              <div className="p-6">
                ...
                <div className="space-y-4">
                  <div>
                    <label className={`block text-xs font-bold uppercase tracking-widest mb-2 ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>Title</label>
                    <input
                      type="text"
                      value={title}
                      ...
                      className={`w-full px-4 py-3 rounded-xl text-sm font-medium outline-none transition-all ${
                        isDark 
                          ? 'bg-slate-800/50 text-white focus:bg-slate-800 focus:ring-2 focus:ring-indigo-500/50' 
                          : 'bg-slate-50 text-slate-900 focus:bg-white focus:ring-2 focus:ring-indigo-500/20 border border-slate-200'
                      }`}
                    />
                  </div>
                  <div>
                    <label className={`block text-xs font-bold uppercase tracking-widest mb-2 ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>Content</label>
                    <textarea
                      value={content}
                  ...
                  <div>
                    <label className={`block text-xs font-bold uppercase tracking-widest mb-2 ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>Reminder (Optional)</label>
                    <input
                      type="datetime-local"
                      value={reminderDate}
                ...
                <div className="flex justify-end gap-3 mt-8">
                  <button 
                    onClick={closeModal}
                    className={`px-5 py-2.5 text-xs font-bold rounded-xl transition-colors ${isDark ? 'text-slate-300 hover:bg-slate-800' : 'text-slate-600 hover:bg-slate-100'}`}
                  >
                    Cancel
                  </button>
                  <button 
