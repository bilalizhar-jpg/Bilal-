...
export function InvoiceProvider({ children}: { children: React.ReactNode}) {
  ...
  const fetchData = async () => {
    ...
    try {
      const [invoicesData, itemsData] = await Promise.all([
        api.get('invoices', { companyId: activeCompanyId }),
        api.get('invoiceItems', { companyId: activeCompanyId })
      ]);

      const sortedInvoices = (invoicesData as Invoice[]).sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
  ...
  const addInvoice = async (
    ...
    for (const item of itemsData) {
      ...
      await api.post('invoiceItems', newItem);
    }
