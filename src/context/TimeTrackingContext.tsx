...
export const TimeTrackingProvider = ({ children}: { children: ReactNode}) => {
  ...
  const fetchTrackingData = async () => {
    if (!user) return;
    try {
      const data = await api.get<TrackingData[]>('/api/timeTracking', { date: todayDate });
      const mappedData: Record<string, TrackingData> = {};
      data.forEach((item: any) => {
        mappedData[item.id] = item;
      });
      setTrackingData(mappedData);
      trackingDataRef.current = mappedData;
    } catch (error) {
  ...
  const updateTracking = React.useCallback(async (employeeId: string, data: Partial<TrackingData>) => {
    ...
    if (statusChanged || now - lastUpdate > 10000) {
      ...
      api.put(`/api/timeTracking/${docId}`, updatedData).catch(error => {
        console.error("Error updating time tracking:", error);
      });
    }
  }, [user?.companyId, user?.currentCompanyId, todayDate]);
  ...
  const startTracking = async (employeeId: string, employeeName: string, companyId: string) => {
    ...
    try {
      await api.put(`/api/timeTracking/${docId}`, newData);
    } catch (error) {
  ...
  const stopTracking = async (employeeId: string) => {
    ...
    setTrackingData(prev => ({ ...prev, [docId]: updatedData}));
    try {
      await api.put(`/api/timeTracking/${docId}`, updatedData);
    } catch (error) {
  ...
  useEffect(() => {
    ...
    // Periodically sync to API (every 10 seconds) to ensure working/idle time is saved
    const syncInterval = setInterval(() => {
      const docId = `${user.employeeId || user.id}_${todayDate}`;
      const current = trackingDataRef.current[docId];
      if (current && current.status !== 'Offline') {
        api.put(`/api/timeTracking/${docId}`, current).catch(err => 
