...
export const AttendanceProvider = ({ children}: { children: ReactNode}) => {
  ...
  const fetchAttendance = async () => {
    ...
    try {
      const data = await api.get<AttendanceRecord[]>('attendance', { companyId: user.companyId });
  ...
  const addLog = async (companyId: string, employeeId: string, employeeName: string, action: string, location: string, time: string, date: string, newState: 'not_checked_in' | 'checked_in' | 'on_break' | 'checked_out', selfie?: string) => {
    ...
    try {
      record = await api.getById('attendance', recordId);
    } catch (e) {
    ...
    if (record) {
      ...
      try {
        await api.put('attendance', recordId, updates);
        fetchAttendance();
      } catch (error) {
