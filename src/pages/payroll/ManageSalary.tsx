...
export default function ManageSalary() {
 ...
 <h2 className="font-bold text-slate-800">Employee salary</h2>
 ...
 <th className="px-4 py-3 text-xs font-bold text-slate-600 uppercase">Total salary</th>
 ...
 <td className="px-4 py-3 text-sm font-medium text-slate-800">{salary.employeeName}</td>
 <td className="px-4 py-3 text-sm text-slate-600">{salary.salaryMonth}</td>
 <td className="px-4 py-3 text-sm text-slate-600">{salary.totalSalary.toFixed(2)}</td>
 ...
 exit={{ opacity: 0, y: 20}}
 className={`rounded-xl shadow-2xl w-full max-w-4xl my-8 overflow-hidden bg-white`}
 >
 <div className="p-4 border-b border-slate-100 flex justify-between items-center sticky top-0 bg-inherit z-10">
 <h3 className="font-bold text-slate-800">Employee Payslip</h3>
 <div className="flex gap-2">
 <button className="p-2 hover:bg-slate-100 rounded-md text-slate-600">
 <Printer className="w-5 h-5"/>
 </button>
 <button onClick={() => setIsPayslipModalOpen(false)} className="p-2 hover:bg-slate-100 rounded-md text-slate-600">
 <X className="w-5 h-5"/>
 </button>
 </div>
 </div>
 ...
 <h2 className="text-2xl font-bold text-slate-800">{currentCompany?.name || 'Bdtask HRM (PAYSLIP)'}</h2>
 </div>
 ...
 <div key={idx} className="flex justify-between border-b border-slate-50 pb-1 items-center group">
 <input 
 type="text"
 value={info.label} 
 onChange={(e) => {
 ...
 className="text-right bg-transparent border-b border-transparent hover:border-slate-300 focus:border-indigo-500 outline-none text-slate-800"
 />
 <button 
 onClick={() => {
 ...
 <table className="w-full text-left border-collapse">
 <thead>
 <tr className="bg-amber-50 border-b border-slate-100">
 <th className="px-4 py-3 text-xs font-bold text-slate-600 uppercase">Description</th>
 <th className="px-4 py-3 text-xs font-bold text-slate-600 uppercase">Amount ({settings.currency.symbol})</th>
 <th className="px-4 py-3 text-xs font-bold text-slate-600 uppercase">Rate ({settings.currency.symbol})</th>
 <th className="px-4 py-3 text-xs font-bold text-slate-600 uppercase">#Value ({settings.currency.symbol})</th>
 <th className="px-4 py-3 text-xs font-bold text-slate-600 uppercase">Deduction ({settings.currency.symbol})</th>
 <th className="px-4 py-3 text-xs font-bold text-slate-600 uppercase w-10 no-print"></th>
 </tr>
 </thead>
 ...
 <input type="text"value={row.description} onChange={(e) => handleRowChange(row.id, 'description', e.target.value)} className="w-full bg-transparent border-b border-transparent hover:border-slate-300 focus:border-indigo-500 outline-none text-slate-700"/>
 </td>
 <td className="px-4 py-2">
 <input type="number"value={row.amount || ''} onChange={(e) => handleRowChange(row.id, 'amount', e.target.value ? Number(e.target.value) : 0)} className="w-24 bg-transparent border-b border-transparent hover:border-slate-300 focus:border-indigo-500 outline-none text-slate-700"/>
 </td>
 <td className="px-4 py-2">
 <input type="number"value={row.rate || ''} onChange={(e) => handleRowChange(row.id, 'rate', e.target.value ? Number(e.target.value) : 0)} className="w-24 bg-transparent border-b border-transparent hover:border-slate-300 focus:border-indigo-500 outline-none text-slate-700"/>
 </td>
 <td className="px-4 py-2">
 <input type="number"value={row.value || ''} onChange={(e) => handleRowChange(row.id, 'value', e.target.value ? Number(e.target.value) : 0)} className="w-24 bg-transparent border-b border-transparent hover:border-slate-300 focus:border-indigo-500 outline-none text-slate-700"/>
 </td>
 ...
 <td className="px-4 py-3 text-slate-800">Total benefit</td>
 <td className="px-4 py-3"></td>
 <td className="px-4 py-3"></td>
 <td className="px-4 py-3 text-slate-800">{totalEarnings.toFixed(2)}</td>
 <td className="px-4 py-3"></td>
 <td className="px-4 py-3 no-print"></td>
 </tr>
 <tr className="bg-slate-50 font-bold">
 <td className="px-4 py-3 text-slate-800 uppercase tracking-wider">Gross salary</td>
 <td className="px-4 py-3"></td>
 <td className="px-4 py-3"></td>
 <td className="px-4 py-3 text-slate-800">{totalEarnings.toFixed(2)}</td>
 <td className="px-4 py-3"></td>
 ...
 <td className="px-4 py-2"></td>
 <td className="px-4 py-2">
 <input type="number"value={row.deduction || ''} onChange={(e) => handleRowChange(row.id, 'deduction', e.target.value ? Number(e.target.value) : 0)} className="w-24 bg-transparent border-b border-transparent hover:border-slate-300 focus:border-indigo-500 outline-none text-slate-700"/>
 </td>
 <td className="px-4 py-2 no-print">
 <button onClick={() => removeRow(row.id)} className="text-red-500 hover:text-red-700 p-1"><Trash2 className="w-4 h-4"/></button>
 </td>
 ...
 <tr className="bg-slate-50 font-bold">
 <td className="px-4 py-3 text-slate-800 uppercase tracking-wider">Total deductions</td>
 <td className="px-4 py-3"></td>
 <td className="px-4 py-3"></td>
 <td className="px-4 py-3"></td>
 <td className="px-4 py-3 text-slate-800">{totalDeductions.toFixed(2)}</td>
 <td className="px-4 py-3 no-print"></td>
 </tr>
 <tr className="bg-amber-50 font-bold text-lg">
 <td className="px-4 py-4 text-slate-800 uppercase tracking-wider">Net salary</td>
 <td className="px-4 py-4"></td>
 <td className="px-4 py-4"></td>
 <td className="px-4 py-4"></td>
 <td className="px-4 py-4 text-slate-800">{netSalary.toFixed(2)}</td>
 ...
 Prepared by: <span className="text-slate-800">Admin</span>
 </div>
 <div className="border-t border-slate-200 pt-2">
 Checked by
 </div>
 ...
 className={`rounded-xl shadow-xl w-full max-w-md overflow-hidden bg-white`}
 >
 <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
 <h3 className="font-bold text-slate-800">Upload Salary Data</h3>
 <button onClick={() => setIsUploadModalOpen(false)} className="p-1 hover:bg-slate-200 rounded-full">
 ...
 <div className="bg-indigo-50 p-8 rounded-2xl border-2 border-dashed border-indigo-200">
