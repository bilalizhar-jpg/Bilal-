...
export default function SalesTaxFormat() {
 ...
 <h1 className="text-2xl font-bold text-slate-800">Certificate of Collection or Deduction of Income Tax</h1>
 ...
 <h3 className="font-bold text-slate-700">Deposit Details</h3>
 {isEditing && (
 ...
 className={`flex-grow border rounded px-3 py-2 text-sm font-bold outline-none focus:ring-1 focus:ring-indigo-500 bg-white border-slate-200`}
 />
 <input 
 type="text"
 value={field.value}
 onChange={(e) => handleCustomFieldChange(field.id, 'value', e.target.value)}
 className={`flex-grow border rounded px-3 py-2 text-sm outline-none focus:ring-1 focus:ring-indigo-500 bg-white border-slate-200`}
 />
 ...
 <p className={`w-full border rounded px-3 py-2 text-sm bg-slate-50 border-slate-200`}>{field.value || 'N/A'}</p>
 </div>
 )}
 </div>
 ))}
...
const InputField: React.FC<InputFieldProps> = ({ label, name, value, onChange, type = 'text', readOnly = false, inline = false, isEditing = false, as = 'input'}) => {
  const commonClass =`border rounded px-3 py-2 text-sm outline-none border-slate-200`;
 const editingClass = isEditing ?`focus:ring-1 focus:ring-indigo-500 bg-white`:`bg-slate-50`;
