...
export default function GmailIntegration() {
  ...
  const IntegrationCard = ({ item }: { item: any }) => (
    <div className={`flex flex-col rounded-xl border ${isDark ? 'bg-[#1E1E2F] border-white/10' : 'bg-white border-slate-200'} overflow-hidden`}>
      <div className="p-6 flex-1">
        <div className="flex items-start gap-4 mb-4">
          <div className={`w-16 h-16 rounded-xl flex items-center justify-center p-2 ${isDark ? 'bg-white/5' : 'bg-slate-50 border border-slate-100'}`}>
            <img src={item.icon} alt={item.name} className="w-full h-full object-contain" referrerPolicy="no-referrer" />
          </div>
          <div className="pt-1">
            <h3 className={`text-lg font-bold ${isDark ? 'text-white' : 'text-slate-800'}`}>{item.name}</h3>
            <a href="#" className="text-[#007BFF] hover:underline text-sm font-medium">Learn more</a>
          </div>
        </div>
        <p className={`text-sm mb-4 ${isDark ? 'text-[#B0B0C3]' : 'text-slate-500'}`}>
          {item.description}
        </p>
        
        {connected[item.id] && connectedEmails[item.id] && (
          <div className={`flex items-center gap-2 text-sm font-medium px-3 py-2 rounded-lg ${isDark ? 'bg-emerald-500/10 text-emerald-400' : 'bg-emerald-50 text-emerald-600'}`}>
      ...
      <button 
        ...
        className={`w-full py-3 text-sm font-bold border-t transition-colors ${
          connected[item.id] 
            ? (isDark ? 'border-white/10 text-red-400 hover:bg-red-500/10' : 'border-slate-200 text-red-600 hover:bg-red-50')
            : (isDark ? 'border-white/10 text-[#00FFCC] hover:bg-[#00FFCC]/10' : 'border-slate-200 text-[#007BFF] hover:bg-blue-50')
        }`}
      >
      ...
      <div className="space-y-10 max-w-5xl relative">
        ...
        <section>
          <div className="flex items-center gap-2 mb-6">
            ...
            <span className={`text-lg ${isDark ? 'text-[#B0B0C3]' : 'text-slate-400'}`}>
          ...
          {isModalOpen && (
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
              <motion.div 
                ...
                className={`w-full max-w-md rounded-xl shadow-xl overflow-hidden ${isDark ? 'bg-[#1E1E2F] border border-white/10' : 'bg-white'}`}
              >
                <div className="p-6">
                  <p className={`text-lg mb-4 ${isDark ? 'text-white' : 'text-[#333333]'}`}>
                    Connect to Gmail.
                  </p>
                  ...
                  <div className="flex justify-end gap-4">
                    <button 
                      ...
                      className={`px-4 py-2 text-sm font-bold ${isDark ? 'text-[#B0B0C3] hover:text-white' : 'text-[#333333] hover:text-black'}`}
                    >
                      Close
                    </button>
                    <button 
