    ...
    <AdminLayout>
      <div className="max-w-4xl">
        <div className="mb-8">
          ...
          <p className={`${isDark ? 'text-[#B0B0C3]' : 'text-slate-500'}`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              ...
              <div className="p-6">
                <div className="flex items-center justify-between mb-6">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className={`text-lg font-bold ${isDark ? 'text-white' : 'text-slate-800'}`}>WhatsApp Status</h3>
                      <div className="flex items-center gap-2">
                        <div className={`w-2 h-2 rounded-full ${status === 'connected' ? 'bg-emerald-500' : 'bg-slate-400'}`} />
                        <span className={`text-sm font-medium ${isDark ? 'text-[#B0B0C3]' : 'text-slate-500'}`}>
                ...
                {status === 'connected' && (
                  <div className={`mt-6 p-4 rounded-lg ${isDark ? 'bg-emerald-500/5 border border-emerald-500/10' : 'bg-emerald-50 border border-emerald-100'}`}>
                    <div className="flex items-center gap-2 text-emerald-600 dark:text-emerald-400 mb-2">
                      <ShieldCheck className="w-4 h-4" />
                      <span className="text-sm font-bold">Secure Connection Active</span>
                    </div>
                    <p className={`text-xs ${isDark ? 'text-emerald-400/60' : 'text-emerald-700/70'}`}>
          ...
          <div className="space-y-6">
            <div className={`p-6 rounded-xl border ${isDark ? 'bg-[#1E1E2F] border-white/10' : 'bg-white border-slate-200'}`}>
              <h3 className={`font-bold mb-4 ${isDark ? 'text-white' : 'text-slate-800'}`}>System Limits</h3>
              <ul className="space-y-3">
                {[
                  ...
                  <li key={i} className="flex items-center gap-2 text-sm">
                    ...
                    <span className={isDark ? 'text-[#B0B0C3]' : 'text-slate-600'}>{limit}</span>
                  </li>
                ))}
              </ul>
            </div>
        ...
        {isModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
            <motion.div
              ...
              className={`w-full max-w-md rounded-2xl shadow-2xl overflow-hidden ${isDark ? 'bg-[#1E1E2F] border border-white/10' : 'bg-white'}`}
            >
              <div className="p-6">
                <div className="flex items-center justify-between mb-6">
                  <h3 className={`text-xl font-bold ${isDark ? 'text-white' : 'text-slate-800'}`}>Scan QR Code</h3>
                  <button 
                    onClick={() => setIsModalOpen(false)}
                    className={`p-2 rounded-lg hover:bg-white/5 transition-colors ${isDark ? 'text-white/40' : 'text-slate-400'}`}
                  >
                    <X className="w-5 h-5" />
                  </button>
                </div>
                ...
                <div className="flex flex-col items-center justify-center py-8">
                  {status === 'connected' ? (
                    ...
                    <div className="w-64 h-64 flex flex-col items-center justify-center gap-4 bg-slate-100 dark:bg-white/5 rounded-xl mb-6">
                  ...
                  <div className="text-center space-y-2">
                    <p className={`font-bold ${isDark ? 'text-white' : 'text-slate-800'}`}>Open WhatsApp on your phone</p>
                ...
                <div className={`mt-6 p-4 rounded-xl text-center text-sm ${isDark ? 'bg-white/5 text-[#B0B0C3]' : 'bg-slate-50 text-slate-600'}`}>
