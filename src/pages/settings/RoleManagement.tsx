...
export default function RoleManagement() {
 ...
 <p className="text-sm text-slate-500 mt-1">Manage user roles and their access levels across the application.</p>
 </div>
 <button 
 ...
 {/* Roles List */}
 <div className={`lg:col-span-1 rounded-xl border bg-white border-slate-200 shadow-sm overflow-hidden flex flex-col h-[calc(100vh-12rem)]`}>
 ...
 className={`w-full pl-9 pr-4 py-2 border rounded-md text-sm outline-none focus:ring-1 focus:ring-indigo-500 bg-slate-50 border-slate-200`}
 />
 </div>
 </div>
 ...
 >
 <div>
 <h3 className={`font-semibold text-sm ${selectedRole?.id === role.id ? 'text-indigo-600 ' : 'text-slate-800 '}`}>
 {role.name}
 {role.isSystem && <span className="ml-2 text-[10px] uppercase tracking-wider bg-slate-200 text-slate-600 px-1.5 py-0.5 rounded">System</span>}
 </h3>
 ...
 {/* Role Details / Edit */}
 <div className={`lg:col-span-2 rounded-xl border bg-white border-slate-200 shadow-sm overflow-hidden flex flex-col h-[calc(100vh-12rem)]`}>
 {selectedRole ? (
 <>
 ...
 className={`text-xl font-bold border-b-2 border-indigo-500 bg-transparent outline-none px-1 py-0.5 text-slate-800`}
 ...
 className={`w-full mt-2 text-sm border rounded px-2 py-1 outline-none bg-white border-slate-300 text-slate-600`}
 ...
 <th className="px-4 py-3 font-semibold text-slate-700 text-center">View</th>
 <th className="px-4 py-3 font-semibold text-slate-700 text-center">Create</th>
 <th className="px-4 py-3 font-semibold text-slate-700 text-center">Edit</th>
 <th className="px-4 py-3 font-semibold text-slate-700 text-center">Delete</th>
 </tr>
 </thead>
 ...
 <td className="px-4 py-3 font-medium text-slate-800 flex items-center justify-between">
 {module}
 {isEditing && !isSuperAdmin && (
 <button 
 onClick={() => toggleAllModulePermissions(module, !allGranted)}
 className={`text-[10px] px-2 py-0.5 rounded-full border ${allGranted ? 'bg-indigo-50 border-indigo-200 text-indigo-600 ' : 'bg-slate-50 border-slate-200 text-slate-500 '}`}
 >
