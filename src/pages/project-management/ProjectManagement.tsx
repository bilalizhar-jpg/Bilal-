...
export default function ProjectManagement() {
 ...
 <TabButton active={activeTab === 'projects'} onClick={() => setActiveTab('projects')} label="Projects"icon={<Briefcase className="w-4 h-4"/>}  />
 <TabButton active={activeTab === 'tasks'} onClick={() => setActiveTab('tasks')} label="Task Board"icon={<Layout className="w-4 h-4"/>}  />
 <TabButton active={activeTab === 'sales'} onClick={() => setActiveTab('sales')} label="Sales & Commission"icon={<DollarSign className="w-4 h-4"/>}  />
 </div>
 ...
 {isAddingProject && (
 <div className={`p-6 rounded-xl border bg-white border-slate-200 shadow-lg space-y-4`}>
 ...
 <InputField label="Company Name"value={newProject.companyName} onChange={(v) => setNewProject({...newProject, companyName: v})}  />
 <InputField label="HR Person Details"value={newProject.hrPersonDetails} onChange={(v) => setNewProject({...newProject, hrPersonDetails: v})}  />
 <InputField label="Contact Person Details"value={newProject.contactPersonDetails} onChange={(v) => setNewProject({...newProject, contactPersonDetails: v})}  />
 <InputField label="Type of Project"value={newProject.projectType} onChange={(v) => setNewProject({...newProject, projectType: v})}  />
 <InputField label="Duration"value={newProject.duration} onChange={(v) => setNewProject({...newProject, duration: v})}  />
 <InputField label="Assigned To"value={newProject.assignedTo} onChange={(v) => setNewProject({...newProject, assignedTo: v})}  />
 <InputField label="Starting Date"type="date"value={newProject.startDate} onChange={(v) => setNewProject({...newProject, startDate: v})}  />
 <InputField label="Ending Date"type="date"value={newProject.endDate} onChange={(v) => setNewProject({...newProject, endDate: v})}  />
 ...
 className={`w-full border rounded px-3 py-2 text-sm bg-white border-slate-200`}
 ...
 >
 <div className="flex justify-between items-center">
 <h3 className="text-lg font-bold text-slate-800">Daily Task Report & Board</h3>
 ...
 <TaskColumn title="To Do"status="To Do"tasks={tasks.filter(t => t.status === 'To Do')} onStatusChange={updateTaskStatus}  onEdit={handleEditTask} onDelete={handleDeleteTask} />
 <TaskColumn title="In Progress"status="In Progress"tasks={tasks.filter(t => t.status === 'In Progress')} onStatusChange={updateTaskStatus}  onEdit={handleEditTask} onDelete={handleDeleteTask} />
 <TaskColumn title="Review"status="Review"tasks={tasks.filter(t => t.status === 'Review')} onStatusChange={updateTaskStatus}  onEdit={handleEditTask} onDelete={handleDeleteTask} />
 <TaskColumn title="Done"status="Done"tasks={tasks.filter(t => t.status === 'Done')} onStatusChange={updateTaskStatus}  onEdit={handleEditTask} onDelete={handleDeleteTask} />
 </div>
 </DragDropContext>
 </motion.div>
 )}
 ...
 >
 <div className="flex justify-between items-center">
 <h3 className="text-lg font-bold text-slate-800">Sales Commission Tracker</h3>
 <button 
 onClick={addSale}
 ...
 <th className="px-4 py-3 text-xs font-bold text-slate-600 uppercase">Seller Name</th>
 <th className="px-4 py-3 text-xs font-bold text-slate-600 uppercase">Product</th>
 <th className="px-4 py-3 text-xs font-bold text-slate-600 uppercase">Sale Amount ({settings.currency.symbol})</th>
 <th className="px-4 py-3 text-xs font-bold text-slate-600 uppercase">Comm. %</th>
 <th className="px-4 py-3 text-xs font-bold text-slate-600 uppercase">Comm. Amount ({settings.currency.symbol})</th>
 <th className="px-4 py-3 text-xs font-bold text-slate-600 uppercase">Pay via Payroll</th>
 ...
 * Commission amounts are automatically calculated based on Sale Amount and Commission Percentage.
 ...
 <h3 className="font-bold text-slate-800">{editingTask.id ? 'Edit Task' : 'New Task'}</h3>
 <button onClick={() => setIsTaskModalOpen(false)} className="p-1 hover:bg-slate-200 rounded-full">
 <X className="w-5 h-5 text-slate-500"/>
 </button>
 </div>
 
 <div className="p-6 space-y-4">
 <InputField label="Task Title"value={editingTask.title} onChange={(v) => setEditingTask({...editingTask, title: v})}  />
 <div>
...
function TaskColumn({ title, status, tasks, onStatusChange, onEdit, onDelete}: any) {
 return (
 <div className={`flex flex-col h-full rounded-xl border bg-slate-50 border-slate-200`}>
 ...
 className={`flex-1 overflow-y-auto p-3 space-y-3 custom-scrollbar ${snapshot.isDraggingOver ? ('bg-slate-100/50') : ''}`}
 >
 ...
 className={`p-3 rounded-lg border bg-white border-slate-200 shadow-sm hover:shadow-md transition-shadow group ${snapshot.isDragging ? 'shadow-lg ring-2 ring-indigo-500 ring-opacity-50' : ''}`}
 >
 ...
 <h5 className="text-sm font-bold text-slate-800 mb-1">{task.title}</h5>
