...
export default function EmployeeMessages() {
  ...
  return (
    <EmployeeLayout>
      <div className={`flex h-[calc(100vh-8rem)] border rounded-xl overflow-hidden ${isDark ? 'border-slate-800 bg-slate-900' : 'border-slate-200 bg-white'}`}>
        ...
        {activeChat ? (
          <div className="flex-1 flex flex-col min-w-0">
            ...
            <div className={`p-4 border-t ${isDark ? 'border-slate-800' : 'border-slate-200'}`}>
              <form onSubmit={handleSendMessage} className={`flex items-end gap-2 p-2 rounded-xl border ${isDark ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-300'} focus-within:ring-2 focus-within:ring-emerald-500 focus-within:border-transparent transition-all`}>
                <textarea
                  value={newMessage}
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-slate-400">
            <div className={`w-16 h-16 mb-4 rounded-full flex items-center justify-center ${isDark ? 'bg-slate-800' : 'bg-slate-100'}`}>
          ...
          <div className={`w-full max-w-md p-6 rounded-xl shadow-xl ${isDark ? 'bg-slate-900 border border-slate-800' : 'bg-white'}`}>
            ...
            <div className="relative mb-4">
              ...
              <input
                ...
                className={`w-full pl-9 pr-4 py-2 rounded-lg border ${isDark ? 'bg-slate-800 border-slate-700 text-white' : 'bg-white border-slate-200'} focus:outline-none focus:ring-2 focus:ring-emerald-500`}
              />
            </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input
              ...
              className={`w-full px-3 py-2 rounded-lg border ${isDark ? 'bg-slate-800 border-slate-700 text-white' : 'bg-white border-slate-200'} focus:outline-none focus:ring-2 focus:ring-emerald-500`}
                ...
                <div
                  ...
                  className={`flex items-center gap-3 p-2 rounded-lg cursor-pointer transition-colors ${
                    selectedUsers.includes(emp.id) 
                      ? (isDark ? 'bg-emerald-900/30 border border-emerald-500/50' : 'bg-emerald-50 border border-emerald-200')
                      : 'hover:bg-slate-100 dark:hover:bg-slate-800'
                  }`}
                >
                  <div className={`w-4 h-4 rounded border flex items-center justify-center ${
                    selectedUsers.includes(emp.id) ? 'bg-emerald-600 border-emerald-600' : 'border-slate-400'
                  }`}>
          ...
          <button
            ...
            className="w-full py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            Create Group
          </button>
        </form>
