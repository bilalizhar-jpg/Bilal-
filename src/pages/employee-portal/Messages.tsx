...
export default function EmployeeMessages() {
 ...
 return (
 <EmployeeLayout>
 <div className={`flex h-[calc(100vh-10rem)] border rounded-3xl overflow-hidden shadow-2xl border-slate-100 bg-white`}>
 
 {/* Sidebar */}
 <div className={`w-80 flex flex-col border-r border-slate-100 bg-slate-50/50`}>
 <div className={`p-6 border-b border-slate-100 flex justify-between items-center`}>
 <h2 className={`font-bold text-xl tracking-tight text-slate-900`}>Messages</h2>
 <div className="flex gap-2">
 <button 
 onClick={() => setShowCreateGroupModal(true)}
 title="Create Group"
 className={`p-2 rounded-xl transition-all hover:scale-110 bg-white text-slate-500 hover:text-indigo-600 shadow-sm`}
 >
 ...
 {chat.type === 'group' ? (
 <div className={`w-10 h-10 rounded-full flex items-center justify-center ${activeChat?.id === chat.id ? 'bg-indigo-500' : ('bg-slate-200')}`}>
 ...
 />
 <div className={`absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full border-2 ${activeChat?.id === chat.id ? 'border-indigo-600' : ('border-white')} ${
 ...
 {chat.lastMessage && (
 <p className={`text-xs truncate ${activeChat?.id === chat.id ? 'text-indigo-100' : ('text-slate-400')}`}>
 ...
 <div className="w-12 h-12 rounded-full bg-slate-100 flex items-center justify-center mx-auto mb-4 opacity-50">
 ...
 {/* Chat Header */}
 <div className={`h-20 px-8 flex items-center justify-between border-b border-slate-100 bg-white/50 backdrop-blur-md`}>
 ...
 className={`p-2.5 rounded-xl transition-all hover:scale-110 bg-white text-slate-500 hover:text-indigo-600 shadow-sm`}
 title="Video Call"
 >
 ...
 className={`p-2.5 rounded-xl transition-all hover:scale-110 bg-white text-slate-500 hover:text-rose-600 shadow-sm`}
 title="Delete Chat"
 >
 ...
 <form onSubmit={handleSendMessage} className={`flex items-end gap-3 p-3 rounded-2xl border transition-all focus-within:ring-4 focus-within:ring-indigo-500/10 bg-white border-slate-200 focus-within:border-indigo-500 shadow-sm`}>
 <textarea
 value={newMessage}
 ...
 className={`flex-1 max-h-32 min-h-[44px] bg-transparent border-none focus:ring-0 resize-none py-2.5 text-sm text-slate-900 placeholder-slate-400`}
 rows={1}
 />
 <button 
 type="submit"
 ...
 <div className={`w-20 h-20 mb-6 rounded-3xl flex items-center justify-center bg-white shadow-xl shadow-slate-200/50`}>
 ...
 <h3 className={`text-2xl font-bold tracking-tight text-slate-900`}>New Chat</h3>
 <button onClick={() => setShowAddUserModal(false)} className="p-2 rounded-xl hover:bg-slate-100 transition-colors">
 ...
 className={`w-full pl-12 pr-4 py-3 rounded-2xl border transition-all focus:ring-4 focus:ring-indigo-500/10 bg-slate-50 border-slate-100 text-slate-900 focus:border-indigo-500`}
 />
 </div>
 ...
 className={`w-full flex items-center gap-4 p-3 rounded-2xl transition-all hover:bg-indigo-50 group`}
 >
 <img 
...
function CreateGroupModal({ onClose, employees, currentUserId, onCreate, }: any) {
 ...
 <div>
 <label className={`block text-[10px] font-bold uppercase tracking-widest mb-2 text-slate-500`}>Group Name</label>
 <input
 type="text"
 value={groupName}
 onChange={(e) => setGroupName(e.target.value)}
 className={`w-full px-4 py-3 rounded-2xl border transition-all focus:ring-4 focus:ring-indigo-500/10 bg-slate-50 border-slate-100 text-slate-900 focus:border-indigo-500`}
 ...
 <div>
 <label className={`block text-[10px] font-bold uppercase tracking-widest mb-2 text-slate-500`}>Add Members</label>
