  ...
  return (
    <EmployeeLayout>
      <div className="space-y-10">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-8 pb-8 border-b border-slate-100 dark:border-slate-800">
          <div className="space-y-2">
            <h1 className={`text-4xl font-bold tracking-tight ${isDark ? 'text-white' : 'text-slate-900'}`}>
              Welcome back, <span className="text-indigo-600 dark:text-indigo-400">{user?.name?.split(' ')[0] || 'Employee'}</span>!
            </h1>
            <p className="text-slate-500 font-medium text-lg">Here's what's happening with your workspace today.</p>
          </div>
          <div className="flex items-center gap-4">
            <div className={`flex items-center gap-2 px-4 py-2 rounded-xl border ${isDark ? 'bg-slate-900 border-slate-800 text-emerald-400' : 'bg-emerald-50 border-emerald-100 text-emerald-600'} text-xs font-bold uppercase tracking-wider`}>
              <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></div>
              System Operational
            </div>
            <div className={`flex items-center gap-2 px-4 py-2 rounded-xl border ${isDark ? 'bg-slate-900 border-slate-800 text-slate-400' : 'bg-slate-50 border-slate-100 text-slate-600'} text-xs font-bold uppercase tracking-wider`}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            {currentEmployee?.notifications && currentEmployee.notifications.length > 0 && (
              <motion.div 
                ...
                className={`p-8 rounded-3xl border ${isDark ? 'bg-slate-900/50 border-slate-800' : 'bg-white border-slate-100 shadow-xl shadow-slate-200/50 dark:shadow-none'}`}
              >
                ...
                <div className="space-y-4">
                  {currentEmployee.notifications.map((notification, idx) => (
                    <div key={idx} className={`flex items-center justify-between p-5 rounded-2xl border transition-all ${isDark ? 'bg-slate-800/50 border-slate-700 hover:border-indigo-500/50' : 'bg-slate-50/50 border-slate-100 hover:border-indigo-500/30'}`}>
                      <div className="flex items-center gap-4">
                        <div className="w-10 h-10 rounded-xl bg-indigo-500/10 flex items-center justify-center text-indigo-500">
                          <Bell className="w-5 h-5" />
                        </div>
                        <p className={`text-sm font-bold ${isDark ? 'text-slate-200' : 'text-slate-700'}`}>{notification}</p>
                      </div>
              ...
              >
                ...
                <div className="space-y-4">
                  {invitations.map(invite => {
                    ...
                    return (
                      <div key={invite.id} className={`flex items-center justify-between p-5 rounded-2xl border transition-all ${isDark ? 'bg-slate-800/50 border-slate-700 hover:border-indigo-500/50' : 'bg-slate-50/50 border-slate-100 hover:border-indigo-500/30'}`}>
                        <div className="flex items-center gap-4">
                        ...
                        <div className="flex gap-2">
                          ...
                          <button 
                            ...
                            className={`px-5 py-2 text-xs font-bold rounded-xl transition-all ${isDark ? 'bg-slate-800 text-slate-400 hover:bg-slate-700' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}
                          >
                            Reject
                          </button>
                        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          {/* Profile Card */}
          {currentEmployee && (
            <div className={`lg:col-span-2 p-10 rounded-3xl border ${isDark ? 'bg-slate-900/50 border-slate-800' : 'bg-white border-slate-100 shadow-xl shadow-slate-200/50 dark:shadow-none'} relative overflow-hidden group`}>
              ...
              <div className="flex items-center justify-between mb-10">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className={`text-sm font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>{currentEmployee.designation}</p>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-10">
                <div className="space-y-1">
                  ...
                  <p className={`text-xl font-bold tracking-tight ${isDark ? 'text-white' : 'text-slate-900'}`}>{currentEmployee.employeeId}</p>
                </div>
                <div className="space-y-1">
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Office Location</p>
                  <p className={`text-xl font-bold tracking-tight ${isDark ? 'text-white' : 'text-slate-900'}`}>{currentEmployee.location || 'Main HQ'}</p>
                </div>
                <div className="space-y-1">
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Reporting To</p>
                  <p className={`text-xl font-bold tracking-tight ${isDark ? 'text-white' : 'text-slate-900'}`}>Admin</p>
                </div>
                <div className="space-y-1">
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Joining Date</p>
                  <p className={`text-xl font-bold tracking-tight ${isDark ? 'text-white' : 'text-slate-900'}`}>{currentEmployee.joiningDate}</p>
                </div>
              </div>
            </div>
          )}
          ...
          <div className={`p-10 rounded-3xl border ${isDark ? 'bg-slate-900/50 border-slate-800' : 'bg-white border-slate-100 shadow-xl shadow-slate-200/50 dark:shadow-none'}`}>
            ...
            <div className="grid grid-cols-1 gap-6">
              <div className={`p-6 rounded-2xl border flex items-center justify-between ${isDark ? 'bg-slate-800/50 border-slate-700' : 'bg-slate-50/50 border-slate-100'}`}>
                <div>
                  <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1">Active Tasks</p>
                  <p className={`text-4xl font-bold tracking-tighter ${isDark ? 'text-white' : 'text-slate-900'}`}>{pendingTasksCount}</p>
                </div>
              ...
              <div className={`p-6 rounded-2xl border flex items-center justify-between ${isDark ? 'bg-slate-800/50 border-slate-700' : 'bg-slate-50/50 border-slate-100'}`}>
                <div>
                  ...
                  <p className={`text-4xl font-bold tracking-tighter ${isDark ? 'text-white' : 'text-slate-900'}`}>{newNoticesCount}</p>
                </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className={`p-10 rounded-3xl border ${isDark ? 'bg-slate-900/50 border-slate-800' : 'bg-white border-slate-100 shadow-xl shadow-slate-200/50 dark:shadow-none'}`}>
            ...
            <div className="grid grid-cols-2 gap-6">
              <motion.button 
                ...
                className={`flex flex-col items-center justify-center p-8 rounded-2xl border transition-all group ${isDark ? 'bg-indigo-500/5 border-indigo-500/20 text-indigo-400 hover:bg-indigo-500/10' : 'bg-indigo-50 border-indigo-100 text-indigo-600 hover:bg-indigo-100'}`}
              >
              ...
              <motion.button 
                ...
                className={`flex flex-col items-center justify-center p-8 rounded-2xl border transition-all group ${isDark ? 'bg-blue-500/5 border-blue-500/20 text-blue-400 hover:bg-blue-500/10' : 'bg-blue-50 border-blue-100 text-blue-600 hover:bg-blue-100'}`}
              >
          ...
          <div className={`p-10 rounded-3xl border ${isDark ? 'bg-slate-900/50 border-slate-800' : 'bg-white border-slate-100 shadow-xl shadow-slate-200/50 dark:shadow-none'} relative overflow-hidden group`}>
            ...
            <div className="space-y-6">
              {recentNotices.map((notice, i) => (
                <div key={i} className="flex items-start gap-4 group/item cursor-pointer" onClick={() => navigate('/employee-portal/notices')}>
                  ...
                  <div>
                    <h4 className={`text-sm font-bold leading-tight group-hover/item:text-indigo-600 transition-colors ${isDark ? 'text-slate-200' : 'text-slate-800'}`}>{notice.type}</h4>
              ...
              <button 
                ...
                className={`w-full mt-6 py-4 text-[10px] font-bold uppercase tracking-[0.2em] border-t transition-all flex items-center justify-center gap-2 ${isDark ? 'border-slate-800 text-indigo-400 hover:text-indigo-300' : 'border-slate-100 text-indigo-600 hover:text-indigo-700'}`}
              >
                View All Notices
        ...
        <div className={`p-10 rounded-3xl border ${isDark ? 'bg-slate-900/50 border-slate-800' : 'bg-white border-slate-100 shadow-xl shadow-slate-200/50 dark:shadow-none'}`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {latestLeave ? (
              <div className={`flex items-center justify-between p-8 rounded-2xl border transition-all ${isDark ? 'bg-slate-800/50 border-slate-700 hover:border-indigo-500/50' : 'bg-slate-50/50 border-slate-100 hover:border-indigo-500/30'}`}>
                <div className="flex items-center gap-5">
                  ...
                  <div>
                    <p className={`text-sm font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>{latestLeave.type}</p>
