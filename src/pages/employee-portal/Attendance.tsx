...
export default function EmployeeAttendance() {
 ...
 className="flex p-1.5 rounded-2xl bg-white border border-slate-200 shadow-sm backdrop-blur-md"
 >
 {[
 ...
 initial={{ opacity: 0, y: 20}}
 animate={{ opacity: 1, y: 0}}
 className={`relative rounded-[2.5rem] p-12 border bg-white border-slate-200 shadow-xl shadow-slate-200/50 backdrop-blur-2xl overflow-hidden`}
 >
 ...
 <div className="flex items-center gap-3 text-slate-500 text-xl font-semibold">
 ...
 >
 <span className="text-sm font-bold text-indigo-600 uppercase tracking-widest mb-2">Active Session</span>
 <div className="text-5xl font-black tabular-nums text-slate-900 tracking-tight">
 ...
 <motion.div 
 whileHover={{ y: -5}}
 className={`p-8 rounded-3xl border transition-all duration-300 bg-white border-slate-200 shadow-sm hover:shadow-md`}
 >
 <div className="flex items-center gap-5 mb-8">
 <div className={`p-4 rounded-2xl ${location ? 'bg-emerald-500/10 text-emerald-600' : 'bg-slate-100 text-slate-400 '}`}>
 ...
 <div className="p-4 rounded-2xl bg-slate-50 border border-slate-100 font-mono text-sm flex justify-between items-center">
 <div className="space-y-1">
 <span className="block text-[10px] uppercase font-bold text-slate-400">Latitude</span>
 <span className="text-slate-700">{location.lat.toFixed(6)}</span>
 </div>
 <div className="w-px h-8 bg-slate-200"/>
 <div className="space-y-1 text-right">
 <span className="block text-[10px] uppercase font-bold text-slate-400">Longitude</span>
 <span className="text-slate-700">{location.lng.toFixed(6)}</span>
 </div>
 </div>
 <div className="flex items-center gap-2.5 px-4 py-2 rounded-xl bg-emerald-500/5 text-xs text-emerald-600 font-bold uppercase tracking-wider border border-emerald-500/10">
 ...
 >
 <div className="flex items-center gap-5 mb-8">
 <div className={`p-4 rounded-2xl ${capturedImage ? 'bg-indigo-500/10 text-indigo-600' : 'bg-slate-100 text-slate-400 '}`}>
 ...
 <div className="px-3 py-1 rounded-lg bg-indigo-500/10 text-[10px] font-bold text-indigo-600 uppercase tracking-widest inline-block">
 Identity Captured
 </div>
 <button
 onClick={async () => {
 ...
 className="w-full py-2.5 rounded-xl border border-indigo-200 text-[10px] font-bold uppercase tracking-widest text-indigo-600 hover:bg-indigo-50 transition-all"
 >
 Sync to Profile
 </button>
 </div>
 ...
 <button onClick={stopCamera} className="flex-1 py-3 rounded-xl bg-slate-100 font-bold text-xs uppercase tracking-widest text-slate-600 hover:bg-slate-200 transition-all">Cancel</button>
 </div>
 </motion.div>
 ) : (
 <button 
 onClick={startCamera} 
 className="w-full py-4 rounded-2xl border-2 border-dashed border-slate-200 font-bold text-xs uppercase tracking-widest text-slate-500 hover:text-indigo-600 hover:border-indigo-500/50 hover:bg-indigo-50/30 transition-all flex flex-col items-center gap-2"
 >
 ...
 ?`border-${action.color}-500/20 bg-white shadow-sm hover:shadow-xl hover:shadow-${action.color}-500/10`
 ...
 action.active && !isPerformingAction ?`text-${action.color}-600 ${action.color}-400`: ''
}`}>
 {action.label}
 </span>
 </motion.button>
 ...
 initial={{ opacity: 0, x: 20}}
 animate={{ opacity: 1, x: 0}}
 className={`rounded-[2.5rem] border bg-white border-slate-200 shadow-xl shadow-slate-200/50 overflow-hidden flex flex-col h-[calc(100vh-16rem)] sticky top-8`}
 >
 ...
 <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-indigo-500/10 text-indigo-600 text-xs font-black">
 ...
 <div className="space-y-10 relative before:absolute before:left-[1.375rem] before:top-2 before:bottom-2 before:w-0.5 before:bg-slate-100">
 {logs.map((log, index) => (
 <motion.div 
 ...
 >
 <div className={`absolute left-0 top-0 w-11 h-11 rounded-2xl flex items-center justify-center z-10 border-4 border-white shadow-sm transition-transform group-hover:scale-110 ${
 ...
 <h4 className="font-black text-sm tracking-tight text-slate-900">{log.action}</h4>
 <span className="text-[10px] font-black text-slate-400 bg-slate-50 px-2 py-1 rounded-md">{log.time}</span>
 </div>
 ...
 animate={{ opacity: 1, scale: 1}}
 className={`rounded-[2.5rem] border bg-white border-slate-200 shadow-xl shadow-slate-200/50 overflow-hidden`}
 >
 ...
 <div className="px-4 py-2 rounded-2xl bg-white border border-slate-200 text-xs font-black text-slate-600 shadow-sm">
 ...
 <div className="p-10 border-b border-slate-100 bg-indigo-600/[0.02]">
 ...
 <div key={i} className="space-y-3 p-6 rounded-3xl bg-white border border-slate-100 shadow-sm">
 <div className="flex items-center gap-3">
 <div className={`p-2 rounded-lg bg-${stat.color}-500/10 text-${stat.color}-600 ${stat.color}-400`}>
 ...
 key={record.id} 
 className="group hover:bg-indigo-600/[0.02] .05] transition-colors"
 >
 <td className="px-10 py-7">
 <div className="flex items-center gap-3">
 <div className="w-10 h-10 rounded-xl bg-slate-100 flex items-center justify-center text-xs font-black text-slate-500 group-hover:bg-indigo-600 group-hover:text-white transition-colors">
 {new Date(record.date).getDate()}
 </div>
 <span className="text-sm font-black text-slate-900 tracking-tight">{record.date}</span>
 </div>
 </td>
 <td className="px-10 py-7">
 ...
 <td className="px-10 py-7">
 <span className="px-4 py-2 rounded-xl bg-indigo-50 text-indigo-600 text-xs font-black tracking-tight border border-indigo-100">
 {record.workingHours}
 </span>
 </td>
 <td className="px-10 py-7">
