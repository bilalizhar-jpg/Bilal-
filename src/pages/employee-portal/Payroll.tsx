...
export default function EmployeePayroll() {
 ...
 <div>
 <h1 className={`text-3xl font-bold tracking-tight text-slate-900`}>Payroll & Finance</h1>
 <p className="text-slate-500 font-medium mt-1">Manage your salary slips, advance requests, and loans.</p>
 </div>
 
 {/* Tabs */}
 <div className={`flex p-1 rounded-2xl border bg-slate-100 border-slate-200`}>
 <button
 ...
 {/* Content Area */}
 <div className={`rounded-3xl border overflow-hidden shadow-xl bg-white border-slate-100`}>
 ...
 <div key={slip.id} className="p-8 flex flex-col lg:flex-row lg:items-center justify-between gap-6 hover:bg-slate-50/50 transition-all group">
 <div className="flex items-center gap-6">
 <div className={`w-14 h-14 rounded-2xl flex items-center justify-center shrink-0 transition-transform group-hover:scale-110 bg-indigo-50 text-indigo-600`}>
 <Calendar className="w-7 h-7"/>
 </div>
 <div>
 <h4 className={`font-bold text-xl tracking-tight text-slate-900`}>{slip.month}</h4>
 ...
 className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-bold transition-all bg-white border border-slate-200 text-slate-700 hover:bg-slate-50 shadow-sm`}
 >
 ...
 <div className="w-16 h-16 rounded-3xl bg-slate-100 flex items-center justify-center mx-auto mb-4 opacity-50">
 ...
 <td className={`px-6 py-5 text-sm font-bold text-slate-900`}>{req.date}</td>
 <td className="px-6 py-5 text-sm font-bold text-indigo-600">{req.amount}</td>
 ...
 <td className={`px-6 py-5 text-sm font-bold text-slate-900`}>{req.remaining}</td>
 <td className="px-6 py-5">
 <span className={`inline-flex items-center px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${
 req.status === 'Active' ? 'bg-indigo-500/10 text-indigo-600 ' :
 ...
 className={`rounded-3xl shadow-2xl w-full max-w-4xl my-8 overflow-hidden bg-white`}
 >
 <div className={`p-6 border-b border-slate-100 flex justify-between items-center sticky top-0 bg-inherit z-10 backdrop-blur-md`}>
 <div className="flex items-center gap-3">
 <div className="p-2 rounded-xl bg-indigo-500/10 text-indigo-500">
 <FileText className="w-5 h-5"/>
 </div>
 <h3 className={`font-bold text-lg tracking-tight text-slate-900`}>Salary Statement - {selectedSlip.month}</h3>
 </div>
 <div className="flex gap-2">
 <button onClick={() => window.print()} className={`p-2.5 rounded-xl transition-all hover:scale-110 bg-slate-100 text-slate-500 hover:text-indigo-600`}>
 <Printer className="w-5 h-5"/>
 </button>
 <button onClick={() => setIsPayslipModalOpen(false)} className={`p-2.5 rounded-xl transition-all hover:scale-110 bg-slate-100 text-slate-500 hover:text-rose-600`}>
 <X className="w-5 h-5"/>
 </button>
 </div>
 </div>
 ...
 <div>
 <h2 className={`text-2xl font-bold tracking-tight text-slate-900`}>{company?.name || 'Corporate HRM'}</h2>
 <p className="text-sm text-slate-500 font-medium">Official Salary Statement</p>
 </div>
 </div>
 </div>
 <div className="text-right space-y-1">
 <p className={`text-3xl font-bold tracking-tighter text-slate-900`}>PAYSLIP</p>
 <p className="text-sm font-bold text-indigo-600 uppercase tracking-widest">{selectedSlip.month}</p>
 </div>
 </div>

 {/* Employee Info Grid */}
 <div className={`grid grid-cols-1 md:grid-cols-2 gap-8 p-8 rounded-3xl border bg-slate-50/50 border-slate-100`}>
 <div className="space-y-4">
 ...
 {/* Salary Table */}
 <div className={`overflow-hidden border rounded-3xl border-slate-100`}>
 ...
 <td className={`px-6 py-4 text-sm font-bold text-slate-700`}>{row.description}</td>
 ...
 <td className={`px-6 py-4 text-sm font-bold text-slate-800`}>{row.value}</td>
 ...
 <td colSpan={2}></td>
 <td className={`px-6 py-4 text-sm text-slate-900`}>{settings.currency.symbol}{totalEarnings.toFixed(2)}</td>
 <td></td>
 </tr>
 ...
 <button onClick={() => setIsPayslipModalOpen(false)} className={`px-6 py-2.5 rounded-xl text-sm font-bold transition-all bg-slate-100 text-slate-600 hover:bg-slate-200`}>
 Close
 </button>
 </div>
 </div>
