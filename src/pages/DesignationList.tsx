...
export default function DesignationList() {
 const { designations, addEntity, updateEntity, deleteEntity} = useCompanyData();
 ...
 const handleOpenModal = (designation: Designation | null = null) => {
 if (designation) {
 ...
 await updateEntity('designations', editingDesignation.id, formData);
} else {
 await addEntity('designations', formData);
}
 handleCloseModal();
};

 const handleDelete = async (id: string) => {
 await updateEntity('designations', id, { isDeleted: true, deletedAt: Date.now()});
};

 const handleRecover = async (id: string) => {
 await updateEntity('designations', id, { isDeleted: false, deletedAt: null});
};

 const handleExportExcel = () => {
 const exportData = filteredDesignations.map((desig, index) => ({
 'Sl': index + 1,
 ...
 doc.setFontSize(16);
 doc.text("Designations List", 14, 20);
 
 const tableColumn = ["Sl","Designation Name","Status"];
 const tableRows = filteredDesignations.map((desig, index) => [
 index + 1,
 desig.name,
 desig.status
 ]);
 ...
 const filteredDesignations = (designations as any[] as Designation[]).filter(d => {
 ...
 <h1 className="text-xl font-bold text-slate-800">Designation list</h1>
 ...
 <th className="px-4 py-3 text-xs font-bold text-slate-600 uppercase border-r border-slate-100">Designation name</th>
 ...
 <tr key={desig.id} className={`transition-colors ${desig.isDeleted ? 'bg-red-50/50' : 'hover:bg-slate-50/50'}`}>
 ...
 {desig.name}
 {desig.isDeleted && <span className="ml-2 text-[10px] font-bold text-red-600 uppercase tracking-wider">(Deleted)</span>}
 </td>
 <td className="px-4 py-3 text-sm border-r border-slate-100">
 <span className={`text-[10px] font-bold px-2 py-1 rounded-full uppercase ${desig.status === 'Active' ? 'bg-emerald-50 text-emerald-600' : 'bg-slate-100 text-slate-500'}`}>
 {desig.status}
 </span>
 </td>
 ...
 {!desig.isDeleted ? (
 <>
 <button onClick={() => handleOpenModal(desig)} className="p-1.5 text-emerald-600 bg-emerald-50 rounded border border-emerald-100 hover:bg-emerald-100"><Edit className="w-3.5 h-3.5"/></button>
 <button onClick={() => handleDelete(desig.id)} className="p-1.5 text-red-600 bg-red-50 rounded border border-red-100 hover:bg-red-100"><Trash2 className="w-3.5 h-3.5"/></button>
 </>
 ) : (
 <button onClick={() => handleRecover(desig.id)} className="flex items-center gap-1.5 px-3 py-1.5 text-indigo-600 bg-indigo-50 rounded border border-indigo-100 hover:bg-indigo-100 text-xs font-bold">
 ...
 <h2 className="font-bold text-slate-800">{editingDesignation ? 'Edit designation' : 'Add designation'}</h2>
 ...
 <div>
 <label className="block text-sm font-bold text-slate-700 mb-1">Designation name <span className="text-red-500">*</span></label>
 <input type="text"name="name"required value={formData.name} onChange={handleInputChange} className="w-full border border-slate-200 rounded px-3 py-2 text-sm outline-none focus:ring-1 focus:ring-indigo-500"placeholder="e.g. Senior Developer"/>
 </div>
 <div>
