...
`);
 ...
 {/* Toolbar */}
 <div className={`h-14 border-b flex items-center justify-between px-4 bg-white border-slate-200`}>
 <div className="flex items-center gap-2">
 <button onClick={handleUndo} disabled={historyIndex === 0} className="p-2 hover:bg-slate-100 rounded disabled:opacity-50"title="Undo">
 <Undo className="w-4 h-4 text-slate-600"/>
 </button>
 <button onClick={handleRedo} disabled={historyIndex === history.length - 1} className="p-2 hover:bg-slate-100 rounded disabled:opacity-50"title="Redo">
 <Redo className="w-4 h-4 text-slate-600"/>
 </button>
 <div className="w-px h-6 bg-slate-200 mx-2"/>
 <button onClick={handleDelete} className="p-2 hover:bg-red-50 rounded text-red-600"title="Delete Selected">
 ...
 className={`p-1.5 rounded ${selectedNode.data.textStyle?.bold ? 'bg-white shadow-sm' : 'hover:bg-white '}`}
 title="Bold"
 >
 ...
 className={`p-1.5 rounded ${selectedNode.data.textStyle?.align === 'left' ? 'bg-white shadow-sm' : 'hover:bg-white '}`}
 title="Align Left"
 >
 ...
 className={`p-1.5 rounded ${(!selectedNode.data.textStyle?.align || selectedNode.data.textStyle?.align === 'center') ? 'bg-white shadow-sm' : 'hover:bg-white '}`}
 title="Align Center"
 >
 ...
 className={`p-1.5 rounded ${selectedNode.data.textStyle?.align === 'right' ? 'bg-white shadow-sm' : 'hover:bg-white '}`}
 title="Align Right"
 >
 ...
 className="hidden"
 />
 <button onClick={() => setIsTemplateModalOpen(true)} className="flex items-center gap-2 px-3 py-1.5 border border-slate-200 rounded text-sm font-medium hover:bg-slate-50 text-slate-600"title="Manage Templates">
 <FolderOpen className="w-4 h-4"/> Templates
 </button>
 <button onClick={() => fileInputRef.current?.click()} className="flex items-center gap-2 px-3 py-1.5 border border-slate-200 rounded text-sm font-medium hover:bg-slate-50 text-slate-600"title="Load Project (JSON)">
 <Upload className="w-4 h-4"/> Load
 </button>
 <button onClick={handleSaveProject} className="flex items-center gap-2 px-3 py-1.5 border border-slate-200 rounded text-sm font-medium hover:bg-slate-50 text-slate-600"title="Save Project (Word)">
 ...
 <button onClick={handlePrint} className="flex items-center gap-2 px-3 py-1.5 border border-slate-200 rounded text-sm font-medium hover:bg-slate-50 text-slate-600">
 ...
 {/* Sidebar / Toolbox */}
 <aside className={`w-64 border-r flex flex-col bg-white border-slate-200`}>
 <div className="p-4 border-b border-slate-100">
 <h3 className="font-bold text-sm text-slate-800">Toolbox</h3>
 <p className="text-xs text-slate-500">Drag shapes to canvas</p>
 </div>
 
 <div className="p-4 grid grid-cols-2 gap-4 overflow-y-auto">
 <div 
 className="flex flex-col items-center gap-2 p-3 border border-slate-200 rounded cursor-grab hover:bg-slate-50 transition-colors"
 ...
 <div 
 className="flex flex-col items-center gap-2 p-3 border border-slate-200 rounded cursor-pointer hover:bg-slate-50 transition-colors"
 ...
 draggable
 >
 <Layout className="w-8 h-8 text-slate-600 transform -skew-x-12"/>
 ...
 <Panel position="top-right"className="bg-white p-2 rounded shadow-sm border border-slate-200 text-xs text-slate-500">
 ...
 <div className={`w-full max-w-md p-6 rounded-xl shadow-2xl bg-white`}>
 ...
 <div className="mb-6">
 <label className="block text-sm font-medium text-slate-700 mb-2">Save Current Chart</label>
 ...
 className="flex-1 px-3 py-2 border border-slate-300 rounded-lg bg-white text-slate-900 focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
 />
 <button 
 ...
 <h3 className="text-sm font-medium text-slate-700 mb-3">Your Templates</h3>
 ...
 <div key={template.id} className="flex items-center justify-between p-3 bg-slate-50 rounded-lg border border-slate-100 hover:border-indigo-200 transition-colors">
 <div>
