...
export default function Payments() {
 ...
 <th className={`py-3 px-4 text-xs font-black uppercase tracking-widest text-slate-500`}>Method</th>
 ...
 const customer = customers.find(c => c.id === payment.customerId);
 return (
 <tr key={payment.id} className={`border-b transition-colors border-slate-100 hover:bg-slate-50`}>
 <td className={`py-3 px-4 text-sm text-slate-600`}>{formatDate(payment.date)}</td>
 ...
 <td className={`py-3 px-4 text-sm font-mono text-right font-bold text-indigo-600`}>
 ...
 className={`relative w-full max-w-2xl max-h-[90vh] flex flex-col rounded-2xl shadow-2xl overflow-hidden bg-white`}
 >
 ...
 <div className={`p-4 rounded-xl border mt-6 bg-slate-50 border-slate-200`}>
