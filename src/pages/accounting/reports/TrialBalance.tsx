  ...
  return (
    <AccountingLayout>
      <div className="space-y-6">
        ...
        {/* Filters */}
        <div className={`p-4 rounded-2xl border ${isDark ? 'bg-[#2A2A3D] border-white/5' : 'bg-white border-slate-200'} flex flex-wrap items-center gap-4`}>
          ...
          <div className="flex items-center gap-2">
            ...
            />
            <span className={isDark ? 'text-white/40' : 'text-slate-400'}>to</span>
            <input
              type="date"
              value={endDate}
        ...
        <div className={`rounded-2xl border overflow-hidden ${isDark ? 'bg-[#2A2A3D] border-white/5' : 'bg-white border-slate-200'}`}>
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className={`border-b ${isDark ? 'border-white/10' : 'border-slate-200'}`}>
                  <th className={`py-4 px-6 text-xs font-black uppercase tracking-widest ${isDark ? 'text-[#B0B0C3]' : 'text-slate-500'}`}>Code</th>
                  <th className={`py-4 px-6 text-xs font-black uppercase tracking-widest ${isDark ? 'text-[#B0B0C3]' : 'text-slate-500'}`}>Account Name</th>
                  <th className={`py-4 px-6 text-xs font-black uppercase tracking-widest text-right ${isDark ? 'text-[#B0B0C3]' : 'text-slate-500'}`}>Debit</th>
                  <th className={`py-4 px-6 text-xs font-black uppercase tracking-widest text-right ${isDark ? 'text-[#B0B0C3]' : 'text-slate-500'}`}>Credit</th>
                </tr>
              </thead>
              <tbody>
                {trialBalance.length === 0 ? (
                  <tr>
                    <td colSpan={4} className={`py-12 text-center text-sm ${isDark ? 'text-[#B0B0C3]' : 'text-slate-500'}`}>
                  ...
                  trialBalance.map((account) => (
                    <tr key={account.id} className={`border-b transition-colors ${isDark ? 'border-white/5 hover:bg-white/5' : 'border-slate-100 hover:bg-slate-50'}`}>
                      <td className={`py-4 px-6 text-sm font-mono ${isDark ? 'text-white' : 'text-slate-900'}`}>{account.code}</td>
                      <td className={`py-4 px-6 text-sm font-medium ${isDark ? 'text-white' : 'text-slate-900'}`}>{account.name}</td>
                      <td className={`py-4 px-6 text-sm font-mono text-right ${isDark ? 'text-emerald-500' : 'text-emerald-600'}`}>
                        {account.totalDebit > 0 ? formatCurrency(account.totalDebit) : '-'}
                      </td>
                      <td className={`py-4 px-6 text-sm font-mono text-right ${isDark ? 'text-rose-500' : 'text-rose-600'}`}>
              ...
              <tfoot>
                <tr className={`bg-slate-50/50 ${isDark ? 'bg-white/5' : 'bg-slate-50'}`}>
                  <td colSpan={2} className={`py-6 px-6 text-sm font-black uppercase tracking-widest text-right ${isDark ? 'text-white' : 'text-slate-900'}`}>
                    Total
                  </td>
                  <td className={`py-6 px-6 text-sm font-mono font-black text-right ${isDark ? 'text-[#00FFCC]' : 'text-indigo-600'}`}>
