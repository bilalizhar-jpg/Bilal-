            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className={`border-b ${isDark ? 'border-white/10' : 'border-slate-200'}`}>
                  <th className={`py-3 px-4 text-xs font-black uppercase tracking-widest ${isDark ? 'text-[#B0B0C3]' : 'text-slate-500'}`}>Bill #</th>
                  ...
                  <th className={`py-3 px-4 text-xs font-black uppercase tracking-widest text-right ${isDark ? 'text-[#B0B0C3]' : 'text-slate-500'}`}>Total</th>
              ...
              <tbody>
                {loading ? (
                  ...
                  filteredBills.map((bill) => {
                    const vendor = vendors.find(v => v.id === bill.vendorId);
                    return (
                      <tr key={bill.id} className={`border-b transition-colors ${isDark ? 'border-white/5 hover:bg-white/5' : 'border-slate-100 hover:bg-slate-50'}`}>
                        <td className={`py-3 px-4 text-sm font-mono ${isDark ? 'text-white' : 'text-slate-900'}`}>{bill.billNumber || 'N/A'}</td>
                        <td className={`py-3 px-4 text-sm font-medium ${isDark ? 'text-white' : 'text-slate-900'}`}>{vendor?.name || 'Unknown Vendor'}</td>
                        <td className={`py-3 px-4 text-sm ${isDark ? 'text-[#B0B0C3]' : 'text-slate-600'}`}>{bill.date}</td>
                        <td className={`py-3 px-4 text-sm ${isDark ? 'text-[#B0B0C3]' : 'text-slate-600'}`}>{bill.dueDate}</td>
                        <td className="py-3 px-4">
                          <span className={`px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${
                            bill.status === 'paid' ? (isDark ? 'bg-emerald-500/20 text-emerald-400' : 'bg-emerald-100 text-emerald-700') :
                            bill.status === 'overdue' ? (isDark ? 'bg-red-500/20 text-red-400' : 'bg-red-100 text-red-700') :
              ...
              <div className="p-6 overflow-y-auto flex-1">
                ...
                <form id="bill-form" onSubmit={handleSubmit} className="space-y-8">
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      ...
                      <button
                        ...
                        className={`flex items-center gap-1 text-xs font-bold uppercase tracking-widest ${isDark ? 'text-[#00FFCC]' : 'text-indigo-600'}`}
                      >
                      ...
                      {items.map((item, index) => (
                        <div key={index} className="grid grid-cols-12 gap-3 items-end">
                          <div className="col-span-12 md:col-span-5">
                            <label className={`block text-[10px] font-black uppercase tracking-widest mb-1 ${isDark ? 'text-[#B0B0C3]' : 'text-slate-500'}`}>Item Description</label>
                            <input
                              type="text"
                              value={item.item}
                              ...
                              className={`w-full px-3 py-2 rounded-lg text-sm transition-all outline-none border ${
                                isDark ? 'bg-[#2A2A3D] border-white/10 text-white focus:border-[#00FFCC]' : 'bg-slate-50 border-slate-200 text-slate-900 focus:border-indigo-500'
                              }`}
                            />
                          </div>
                          <div className="col-span-3 md:col-span-1">
                            <label className={`block text-[10px] font-black uppercase tracking-widest mb-1 ${isDark ? 'text-[#B0B0C3]' : 'text-slate-500'}`}>Qty</label>
                            <input
                              type="number"
                              value={item.quantity}
                          ...
                          <div className="col-span-4 md:col-span-2">
                            <label className={`block text-[10px] font-black uppercase tracking-widest mb-1 ${isDark ? 'text-[#B0B0C3]' : 'text-slate-500'}`}>Price</label>
                            <input
                              type="number"
                              value={item.price}
                          ...
                          <div className="col-span-3 md:col-span-1">
                            <label className={`block text-[10px] font-black uppercase tracking-widest mb-1 ${isDark ? 'text-[#B0B0C3]' : 'text-slate-500'}`}>Tax</label>
                            <input
                              type="number"
                              value={item.tax}
                          ...
                          <div className="col-span-4 md:col-span-2">
                            <label className={`block text-[10px] font-black uppercase tracking-widest mb-1 ${isDark ? 'text-[#B0B0C3]' : 'text-slate-500'}`}>Total</label>
                            <div className={`w-full px-3 py-2 rounded-lg text-sm border font-mono ${
                              isDark ? 'bg-[#1E1E2F] border-white/10 text-[#00FFCC]' : 'bg-slate-100 border-slate-200 text-indigo-600'
                            }`}>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-6 border-t border-white/10">
                    <div className="space-y-4">
                      <h3 className={`text-xs font-bold uppercase tracking-widest ${isDark ? 'text-[#B0B0C3]' : 'text-slate-500'}`}>
                        Accounting Posting
                      </h3>
                      <div className="space-y-4">
                        <div>
                          <label className={`block text-[10px] font-black uppercase tracking-widest mb-1 ${isDark ? 'text-[#B0B0C3]' : 'text-slate-500'}`}>Expense Account (Debit)</label>
                          <select
                            required
                        ...
                        <div>
                          <label className={`block text-[10px] font-black uppercase tracking-widest mb-1 ${isDark ? 'text-[#B0B0C3]' : 'text-slate-500'}`}>Accounts Payable (Credit)</label>
                          <select
                            required
                    ...
                    <div className={`p-6 rounded-2xl space-y-3 ${isDark ? 'bg-white/5' : 'bg-slate-50'}`}>
                      <div className="flex justify-between text-sm">
                        <span className={isDark ? 'text-[#B0B0C3]' : 'text-slate-500'}>Subtotal</span>
                        <span className={isDark ? 'text-white' : 'text-slate-900'}>{formatCurrency(totalAmount - totalTax)}</span>
                      </div>
                      <div className="flex justify-between text-sm">
                        <span className={isDark ? 'text-[#B0B0C3]' : 'text-slate-500'}>Tax</span>
                        <span className={isDark ? 'text-white' : 'text-slate-900'}>{formatCurrency(totalTax)}</span>
                      </div>
                      <div className={`flex justify-between pt-3 border-t-2 ${isDark ? 'border-white/10' : 'border-slate-200'}`}>
                        <span className={`text-lg font-black uppercase tracking-widest ${isDark ? 'text-white' : 'text-slate-900'}`}>Total Amount</span>
                        <span className={`text-xl font-mono font-black ${isDark ? 'text-[#00FFCC]' : 'text-indigo-600'}`}>{formatCurrency(totalAmount)}</span>
                      </div>
                    </div>
                  </div>
                </form>
