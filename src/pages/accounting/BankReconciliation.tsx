...
export default function BankReconciliation() {
 ...
 <p className={`text-xs mt-1 text-slate-400`}>Ref: {tx.reference}</p>
 </motion.div>
 ))}
 ...
 <p className={`text-xs mt-1 text-slate-400`}>No unreconciled transactions for this account.</p>
 </div>
 )}
 </div>
 </div>
 ...
 {!selectedBankTx ? (
 <div className={`p-12 text-center rounded-2xl border border-dashed border-slate-200`}>
 <ArrowRightLeft className={`w-8 h-8 mx-auto mb-3 text-slate-300`} />
 <p className={`text-sm font-bold text-slate-400`}>Select a transaction from the left to find a match</p>
 </div>
 ) : (
 <div className="space-y-3">
 ...
 <p className={`text-xs mt-1 text-slate-400`}>Ref: {entry.reference}</p>
 ...
 <p className={`text-sm font-bold text-slate-900`}>No matching entries found</p>
 <p className={`text-xs mt-1 text-slate-400`}>You may need to create a journal entry for this transaction first.</p>
 </div>
 )}
 </div>
 )}
