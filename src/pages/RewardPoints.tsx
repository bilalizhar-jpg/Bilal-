...
export default function RewardPoints() {
 ...
 <p className={`text-sm text-slate-500`}>Employee Recognition & Rewards</p>
 </div>
 <button
 ...
 <h3 className={`font-bold text-slate-800`}>Total Points Distributed</h3>
 </div>
 <p className={`text-3xl font-bold text-slate-800`}>{(totalPoints || 0).toLocaleString()}</p>
 ...
 <h2 className={`font-bold text-slate-800`}>Reward Leaderboard</h2>
 </div>
 <div className="p-6">
 <div className="space-y-4">
 {leaderboard.length === 0 ? (
 <p className={`text-center py-8 text-slate-400`}>No points awarded yet.</p>
 ) : (
 leaderboard.map((user) => (
 <div key={user.id} className={`flex items-center justify-between p-4 rounded-xl border bg-slate-50 border-slate-100`}>
 ...
 <div className="text-lg font-bold text-indigo-600">{user.rewardPoints}</div>
 ...
 <h2 className={`text-lg font-bold text-slate-800`}>Award Points</h2>
 ...
 <div>
 <label className={`block text-sm font-medium mb-1 text-slate-700`}>Select Employee</label>
 <select
 required
