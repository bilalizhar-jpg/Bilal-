    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div className="w-80 p-6 rounded-xl shadow-xl bg-[#1E1E2F] border border-white/5 text-white">
        ...
        <div className="space-y-4 max-h-[60vh] overflow-y-auto pr-2">
          <div>
            <button className="flex items-center gap-2 font-black mb-2 uppercase text-xs text-[#B0B0C3]" onClick={() => setIsCompanyOpen(!isCompanyOpen)}>
            ...
            {isCompanyOpen && (
              <div className="space-y-2 ml-6">
                ...
                {filteredCompanies.map((comp: string) => (
                  <label key={comp} className="flex items-center gap-2 text-sm text-white">
          ...
          <div>
            <button className="flex items-center gap-2 font-black mb-2 uppercase text-xs text-[#B0B0C3]" onClick={() => setIsStageOpen(!isStageOpen)}>
              {isStageOpen ? <ChevronDown className="w-4 h-4" /> : <ChevronRight className="w-4 h-4" />} Stage
            </button>
            {isStageOpen && (
              <div className="space-y-2 ml-6">
                {stages.map((stage: string) => (
                  <label key={stage} className="flex items-center gap-2 text-sm text-white">
        ...
        <div className="flex gap-2 mt-6">
          <button onClick={() => { setSelectedCompanies([]); setSelectedStages([]); setSelectedStatuses([]); onApply([], [], []); }} className="flex-1 py-2 border border-white/10 rounded-md hover:bg-[#2A2A3D] text-[#B0B0C3]">Reset</button>
          <button onClick={() => onApply(selectedCompanies, selectedStages, selectedStatuses)} className="flex-1 py-2 bg-[#00FFCC] text-[#1E1E2F] rounded-md font-black uppercase hover:bg-[#00D1FF]">Filter</button>
        </div>
      </div>
    </div>
  );
        ...
        <div className="space-y-4 max-h-[60vh] overflow-y-auto pr-2">
          {columns.map((col: any) => (
            <div key={col.id} className="flex justify-between items-center">
              <span className="flex items-center gap-2 text-sm font-medium text-[#B0B0C3]">
                <MoreVertical className="w-4 h-4" /> {col.label}
              </span>
              <button 
    ...
    <AdminLayout>
      ...
      <div className="space-y-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-black text-white uppercase tracking-tight">
              Opportunities <span className="text-sm font-normal text-[#B0B0C3] bg-[#2A2A3D] px-2 py-0.5 rounded-full">{filteredOpportunities.length}</span>
            </h1>
            <p className="text-sm text-[#B0B0C3]">Home / Opportunities</p>
          </div>
        ...
        <div className="flex flex-col md:flex-row justify-between gap-4">
          ...
          <button onClick={() => navigate('/crm/opportunities/add')} className="flex items-center gap-2 px-4 py-2 bg-[#00FFCC] text-[#1E1E2F] rounded-md hover:bg-[#00D1FF] text-sm font-black uppercase shadow-[0_0_8px_rgba(0,255,204,0.4)]">
            ...
            <thead className="text-xs uppercase bg-[#1E1E2F] text-[#B0B0C3]">
              <tr>
                {columns.filter(c => c.visible).map(col => (
                  <th key={col.id} className="py-3 px-4 font-black tracking-wider">{col.label}</th>
                ))}
              </tr>
            </thead>
            <tbody className="text-white">
              {filteredOpportunities.map((opp: any) => (
                <tr key={opp.id} className="border-b border-white/5 hover:bg-[#1E1E2F]/50">
                  {columns.filter(c => c.visible).map(col => (
