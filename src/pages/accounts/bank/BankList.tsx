...
import { Plus, FolderPlus, Search, SlidersHorizontal, FileText, FileSpreadsheet, Printer, Edit2, Trash2 } from 'lucide-react';
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <button className={`flex items-center gap-2 px-4 py-2.5 rounded-md border border-[#6f42c1] text-[#6f42c1] hover:bg-[#6f42c1]/10 transition-all ${isDark ? 'bg-black/40' : 'bg-white'}`}>
            <SlidersHorizontal className="w-4 h-4" />
            Filter
          </button>
          <button onClick={exportToPDF} className={`p-2.5 rounded-md border border-[#fd7e14] text-[#fd7e14] hover:bg-[#fd7e14]/10 transition-all ${isDark ? 'bg-black/40' : 'bg-white'}`}>
            <FileText className="w-4 h-4" />
          </button>
          <button onClick={exportToExcel} className={`p-2.5 rounded-md border border-[#20c997] text-[#20c997] hover:bg-[#20c997]/10 transition-all ${isDark ? 'bg-black/40' : 'bg-white'}`}>
            <FileSpreadsheet className="w-4 h-4" />
          </button>
          <button onClick={() => window.print()} className={`p-2.5 rounded-md border border-[#6f42c1] text-[#6f42c1] hover:bg-[#6f42c1]/10 transition-all ${isDark ? 'bg-black/40' : 'bg-white'}`}>
                ...
                <tr key={bank.id} className={`group transition-colors ${isDark ? 'hover:bg-white/5' : 'hover:bg-slate-50'}`}>
                  ...
                  <td className="px-4 py-4">
                    <span className={`px-2 py-1 rounded text-xs font-medium ${
                      bank.status === 'Option 1' || bank.status === 'Active'
