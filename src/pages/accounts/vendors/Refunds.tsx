...
import { Search, Plus, Filter, Download, MoreHorizontal, FileText } from 'lucide-react';
...
export default function Refunds() {
  ...
  const refunds = [
    { id: 'VREF-001', vendor: 'Office Supplies Co', date: '2023-10-25', total: 120.00, status: 'Posted' },
    { id: 'VREF-002', vendor: 'Tech Solutions Inc', date: '2023-10-26', total: 4500.00, status: 'Draft' },
  ];
    ...
    <AdminLayout>
      <div className="space-y-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h1 className={`text-2xl font-bold ${isDark ? 'text-white' : 'text-slate-800'}`}>Refunds</h1>
            <p className={`text-sm ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>Manage vendor refunds and credits</p>
          </div>
            ...
            <table className="w-full">
              <thead>
                <tr className={`border-b ${isDark ? 'border-slate-700' : 'border-slate-200'}`}>
                  ...
                  <th className={`px-4 py-3 text-left text-sm font-semibold ${isDark ? 'text-slate-300' : 'text-slate-700'}`}>Vendor</th>
                  ...
                  >
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        ...
                        <span className={`text-sm font-medium ${isDark ? 'text-indigo-400' : 'text-indigo-600'}`}>{refund.id}</span>
                      </div>
                    </td>
                    <td className="px-4 py-3">
                      <span className={`text-sm ${isDark ? 'text-slate-300' : 'text-slate-700'}`}>{refund.vendor}</span>
                    </td>
                    <td className="px-4 py-3">
                      <span className={`text-sm ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>{refund.date}</span>
                    </td>
                    <td className="px-4 py-3 text-right">
                      <span className={`text-sm font-bold ${isDark ? 'text-slate-300' : 'text-slate-700'}`}>
                        ${refund.total.toLocaleString(undefined, { minimumFractionDigits: 2 })}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-center">
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                        refund.status === 'Posted' ? 'bg-emerald-100 text-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-400' :
