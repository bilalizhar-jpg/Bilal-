...
export default function CreditNotes() {
  ...
  const creditNotes = [
    { id: 'RCN-001', customer: 'Acme Corp', date: '2023-10-25', total: 1200.00, status: 'Posted' },
    { id: 'RCN-002', customer: 'Globex Inc', date: '2023-10-26', total: 450.00, status: 'Draft' },
    { id: 'RCN-003', customer: 'Soylent Corp', date: '2023-10-27', total: 890.00, status: 'Posted' },
  ];
    ...
    <AdminLayout>
      <div className="space-y-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h1 className={`text-2xl font-bold ${isDark ? 'text-white' : 'text-slate-800'}`}>Credit Notes</h1>
            <p className={`text-sm ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>Manage customer returns and credits</p>
          </div>
                  ...
                  >
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        ...
                        <span className={`text-sm font-medium ${isDark ? 'text-indigo-400' : 'text-indigo-600'}`}>{note.id}</span>
                      </div>
                    </td>
                    <td className="px-4 py-3">
                      <span className={`text-sm ${isDark ? 'text-slate-300' : 'text-slate-700'}`}>{note.customer}</span>
                    </td>
                    <td className="px-4 py-3">
                      <span className={`text-sm ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>{note.date}</span>
                    </td>
                    <td className="px-4 py-3 text-right">
                      <span className={`text-sm font-bold ${isDark ? 'text-slate-300' : 'text-slate-700'}`}>
                        ${note.total.toLocaleString(undefined, { minimumFractionDigits: 2 })}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-center">
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                        note.status === 'Posted' ? 'bg-emerald-100 text-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-400' :
