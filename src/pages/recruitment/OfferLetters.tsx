  ...
  return (
    <AdminLayout>
      <div className="mb-6 flex justify-between items-center">
        <div>
          <h1 className={`text-2xl font-bold ${isDark ? 'text-white' : 'text-slate-800'}`}>Offer Letters</h1>
          <p className={`text-sm ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>Manage and customize offer letter templates</p>
        </div>
        <button
          onClick={() => {
            setIsCreating(true);
      ...
      {editingTemplate ? (
        <div className={`p-6 rounded-xl border ${isDark ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'}`}>
          ...
          <p className="text-xs text-slate-500">Tip: You can type placeholders like [Candidate Name] to replace later.</p>
        </div>
      ) : (
