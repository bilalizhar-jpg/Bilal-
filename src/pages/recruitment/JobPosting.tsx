  ...
  return (
    <AdminLayout>
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <form onSubmit={(e) => handleSubmit(e, 'published')} className={`${isDark ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'} border rounded-xl overflow-hidden`}>
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <input 
                  ...
                  className={`w-full pl-10 pr-4 py-2.5 rounded-lg border focus:ring-2 focus:ring-indigo-500 outline-none transition-all ${
                    isDark 
                      ? 'bg-slate-800 border-slate-700 text-white placeholder-slate-500' 
                      : 'bg-white border-slate-300 text-slate-900 placeholder-slate-400'
                  }`}
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-4">
                  <label className="flex items-center gap-2 cursor-pointer">
                    ...
                    />
                    <span className={`text-sm ${isDark ? 'text-slate-300' : 'text-slate-700'}`}>Yes</span>
                  </label>
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input 
                      type="radio" 
                      ...
                      className="w-4 h-4 text-indigo-600 focus:ring-indigo-500"
                    />
                    <span className={`text-sm ${isDark ? 'text-slate-300' : 'text-slate-700'}`}>No</span>
                  </label>
                </div>
              </div>
                  ...
                  {customQuestions.length > 0 && (
                    <div className="space-y-2 mb-3">
                      {customQuestions.map((q, index) => (
                        <div key={q.id} className={`flex items-center justify-between p-3 rounded-lg border ${isDark ? 'bg-slate-800 border-slate-700' : 'bg-slate-50 border-slate-200'}`}>
                          <div className="flex items-center gap-3">
                            <span className={`text-sm font-medium ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
                              Q{index + 1}:
                            </span>
                            <div>
                              <p className={`text-sm ${isDark ? 'text-white' : 'text-slate-800'}`}>{q.question}</p>
            ...
            <div className="space-y-4">
              ...
              {testMcqs.map((mcq, qIndex) => (
                <div key={mcq.id} className={`p-4 rounded-xl border ${isDark ? 'bg-slate-800/50 border-slate-700' : 'bg-slate-50 border-slate-200'} space-y-4`}>
                  <div className="flex items-start gap-3">
                    <div className="flex-1 space-y-2">
                      <div className="flex items-center justify-between">
                        <span className={`text-xs font-bold uppercase tracking-wider ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>Question {qIndex + 1}</span>
                        <button
                          type="button"
                  ...
                  <div className="space-y-2 ml-4">
                    <div className="flex items-center justify-between">
                      <span className={`text-xs font-medium ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>Options</span>
                      <button
                        type="button"
                      ...
                      {mcq.options.map((option, oIndex) => (
                        <div key={option.id} className="flex items-center gap-2">
                          <input
                            ...
                            className={`flex-1 px-3 py-1.5 text-sm rounded-lg border focus:ring-2 focus:ring-indigo-500 outline-none transition-all ${
                              isDark 
              ...
              {testMcqs.length === 0 && (
                <div className={`text-center py-6 rounded-xl border-2 border-dashed ${isDark ? 'border-slate-800 text-slate-500' : 'border-slate-100 text-slate-400'}`}>
          ...
          <div className={`p-6 border-t ${isDark ? 'border-slate-800 bg-slate-900/50' : 'border-slate-100 bg-slate-50'} flex justify-end gap-3`}>
            <button 
              type="button"
              onClick={handleReset}
              className={`mr-auto flex items-center gap-2 px-4 py-2.5 rounded-lg text-sm font-medium transition-colors ${
                isDark 
