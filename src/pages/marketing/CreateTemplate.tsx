...
export default function CreateTemplate() {
 ...
 className="px-4 py-2 bg-white text-slate-700 border border-slate-200 rounded-md text-sm font-medium hover:bg-slate-50 transition-colors"
 >
 Back to Menu
 </button>
 <button 
 ...
 className="text-2xl font-bold text-slate-800 bg-transparent border-none focus:ring-0 p-0 outline-none"
 />
 <p className="text-sm text-slate-500 mt-1">Drag and drop blocks to build your email</p>
 </div>
 <div className="flex gap-2">
 <button 
 onClick={() => setIsPreviewOpen(true)}
 className="px-4 py-2 bg-white text-slate-700 border border-slate-200 rounded-md text-sm font-medium hover:bg-slate-50 transition-colors flex items-center gap-2"
 >
 ...
 {/* Sidebar Tools */}
 <div className="w-80 bg-white rounded-lg border border-slate-200 shrink-0 flex flex-col">
 ...
 className="flex flex-col items-center justify-center p-4 border border-slate-200 rounded-lg bg-white hover:border-indigo-500 hover:shadow-sm cursor-grab active:cursor-grabbing transition-all group"
 >
 <div className={`mb-2 text-emerald-600 ${block.dashed ? 'border border-dashed border-emerald-600 p-1 rounded' : ''}`}>
 <Icon className="w-6 h-6"/>
 </div>
 <span className="text-xs font-medium text-slate-600 group-hover:text-indigo-600 text-center">
 {block.label}
 </span>
 </div>
 );
 ...
 {/* Canvas */}
 <div 
 className="flex-1 bg-slate-50 rounded-lg border border-slate-200 overflow-y-auto custom-scrollbar p-8 flex justify-center bg-[url('data:image/svg+xml;base64,PHN2ZyB3aWR0aD0iMjAiIGhlaWdodD0iMjAiIHhtbG5zPSJodHRwOi8vd3d3LnczLm9yZy8yMDAwL3N2ZyI+PGNpcmNsZSBjeD0iMiIgY3k9IjIiIHI9IjEiIGZpbGw9IiNlMmU4ZjAiLz48L3N2Zz4=')]"
 onDragOver={handleDragOver}
 onDrop={handleCanvasDrop}
 >
 <div className="w-full max-w-3xl bg-white shadow-sm min-h-full pb-20">
 ...
 <div className="p-1 bg-white shadow-sm rounded border border-slate-200">
 ...
 value={block.content as string}
 onChange={(e) => handleBlockEdit(block.id, e.target.value)}
 className="w-full text-2xl font-bold text-center text-slate-800 bg-transparent border-none focus:ring-2 focus:ring-indigo-500 p-2 outline-none rounded"
 />
 )}
 {block.type === 'image' && (
 <div className="relative bg-slate-100 p-8 flex justify-center items-center min-h-[200px]">
 {block.content ? (
 ...
 <div key={col.id} className="relative bg-slate-100 p-8 flex justify-center items-center min-h-[200px]">
 {col.content ? (
 ...
 {block.type === 'text' && (
 <div className="w-full text-slate-600 bg-transparent border-none focus-within:ring-2 focus-within:ring-indigo-500 p-2 outline-none rounded"onClick={(e) => e.stopPropagation()}>
 <Editor 
 value={block.content as string} 
 onChange={(e) => handleBlockEdit(block.id, e.target.value)}
 ...
 {block.type === 'footer' && (
 <div className="bg-slate-100 p-8 text-center text-xs text-slate-500 mt-8">
 <p className="font-bold text-slate-700 text-sm mb-1">{(block.content as any).company}</p>
 <p>{(block.content as any).address}</p>
 <p className="mt-4">
 This email was sent to <span className="border border-slate-300 px-1 rounded bg-white">{(block.content as any).email}</span>
 </p>
 ...
 <div className="bg-slate-100 p-8 text-center border border-dashed border-slate-300 rounded">
 ...
 {activeBlock === block.id && (
 <div className="mt-2 p-3 bg-white border border-slate-200 rounded shadow-sm text-left">
 <label className="block text-xs font-medium text-slate-700 mb-1">Video URL (YouTube, Vimeo, etc.)</label>
 <input 
 type="text"
 ...
 return (
 <a key={network} href={url || '#'} onClick={e => e.preventDefault()} className={`w-10 h-10 rounded-full flex items-center justify-center ${isActive ? 'bg-indigo-100 text-indigo-600 ' : 'bg-slate-100 text-slate-400 '}`}>
 ...
 {activeBlock === block.id && (
 <div className="mt-4 p-4 bg-slate-50 rounded-lg grid grid-cols-2 gap-3 text-left">
 {['facebook', 'twitter', 'instagram', 'linkedin'].map(network => (
 <div key={`input-${network}`}>
 <label className="block text-xs font-medium text-slate-700 mb-1 capitalize">{network}</label>
 <input 
 type="text"
 ...
 <label className="block text-xs font-medium text-slate-700 mb-1">Custom HTML</label>
 <textarea 
 value={block.content as string}
 onChange={(e) => handleBlockEdit(block.id, e.target.value)}
 className="w-full text-sm font-mono border-slate-300 rounded-md h-32 p-2"
 />
 </div>
 ) : (
 <div 
 ...
 <a key={i} href={navItem.url || '#'} onClick={e => e.preventDefault()} className="text-sm font-medium text-slate-600 hover:text-indigo-600">
 {navItem.label}
 </a>
 ))}
 </div>
 ...
 placeholder="URL"
 className="flex-1 text-xs border-slate-300 rounded p-1.5"
 />
 <button 
 onClick={(e) => {
 e.stopPropagation();
 ...
 <div className="w-32 h-32 shrink-0 bg-slate-100 rounded-lg flex items-center justify-center relative group/productimg">
 ...
 className="w-full text-lg font-bold border-slate-300 rounded p-1.5"
 ...
 className="w-full text-md text-slate-600 border-slate-300 rounded p-1.5"
 placeholder="Price"
 />
 <div className="flex gap-2">
 <input 
 ...
 className="w-1/2 text-sm border-slate-300 rounded p-1.5"
 ...
 <>
 <h3 className="text-lg font-bold text-slate-800 mb-1">{(block.content as any)?.title}</h3>
 <p className="text-md text-slate-600 mb-4">{(block.content as any)?.price}</p>
 ...
 {activeBlock === block.id && (
 <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white p-2 rounded shadow-lg border border-slate-200 flex items-center gap-2 z-20"onClick={e => e.stopPropagation()}>
 ...
 className="w-20 text-xs border-slate-300 rounded p-1"
 min="10"
 max="200"
 />
 </div>
 ...
 <div className="inline-block px-4 py-2 bg-amber-50 text-amber-700 border border-amber-200 rounded font-mono text-sm">
 {block.content as string}
 </div>
 {activeBlock === block.id && (
 <div className="mt-4 p-3 bg-white border border-slate-200 rounded shadow-sm max-w-xs mx-auto text-left">
 <label className="block text-xs font-medium text-slate-700 mb-1">Select Variable</label>
 <select 
 value={block.content as string}
 onChange={(e) => handleBlockEdit(block.id, e.target.value)}
 className="w-full text-sm border-slate-300 rounded-md p-2"
 ...
 <button className="p-1.5 bg-white shadow-sm rounded border border-slate-200 text-slate-500 hover:text-indigo-600">
 ...
 className="p-1.5 bg-white shadow-sm rounded border border-slate-200 text-slate-500 hover:text-red-600"
 >
 ...
 <div 
 className="p-8 border-2 border-dashed border-transparent hover:border-indigo-300 transition-colors flex justify-center items-center text-slate-400"
 onDragOver={handleDragOver}
 onDrop={(e) => {
 e.stopPropagation();
 ...
 <h3 className="text-lg font-bold text-slate-900">Preview: {templateName}</h3>
 <button 
 ...
 {block.type === 'title' && (
 <h1 className="text-2xl font-bold text-center text-slate-800">{block.content as string}</h1>
 )}
 {block.type === 'image' && (
 ...
 <div className="bg-slate-100 w-full h-48 flex items-center justify-center text-slate-400">Image Placeholder</div>
 )}
 </div>
 )}
 {block.type === 'columns' && (
 ...
 <div className="bg-slate-100 w-full h-48 flex items-center justify-center text-slate-400">Image</div>
 )}
 </div>
 ))}
 </div>
 ...
 {block.type === 'text' && (
 <div 
 className="text-slate-600 text-center whitespace-pre-wrap"
 dangerouslySetInnerHTML={{ __html: block.content as string}}
 />
 )}
 {block.type === 'button' && (
 ...
 <div className="flex justify-center">
 {!(block.content as any)?.url ? (
 <div className="bg-slate-100 w-full h-48 flex items-center justify-center text-slate-400">Video Placeholder</div>
 ) : (
 ...
 const Icon = network === 'facebook' ? Facebook : network === 'twitter' ? Twitter : network === 'instagram' ? Instagram : Linkedin;
 return (
 <a key={network} href={url} target="_blank"rel="noopener noreferrer"className="w-10 h-10 rounded-full flex items-center justify-center bg-indigo-100 text-indigo-600">
 <Icon className="w-5 h-5"/>
 </a>
 );
})}
 ...
 <div className="flex justify-center gap-6">
 {((block.content as any[]) || []).map((navItem, i) => (
 <a key={i} href={navItem.url || '#'} className="text-sm font-medium text-slate-600 hover:text-indigo-600">
 {navItem.label}
 </a>
 ))}
 </div>
 ...
 {block.type === 'product' && (
 <div className="p-4 flex flex-col sm:flex-row gap-6 items-center sm:items-start">
 <div className="w-32 h-32 shrink-0 bg-slate-100 rounded-lg flex items-center justify-center">
 {(block.content as any)?.image ? (
 <img src={(block.content as any).image} alt="Product"className="w-full h-full object-cover rounded-lg"/>
 ) : (
 <ShoppingBag className="w-8 h-8 text-slate-400"/>
