    ...
    <AdminLayout>
      <div className="space-y-6">
        <div className="flex justify-between items-center">
          <h1 className="text-2xl font-bold text-slate-800 dark:text-white">Email Campaigns</h1>
          <button 
        ...
        <div className="bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700 p-6">
          <div className="flex gap-4 mb-6">
            <div className="relative w-64">
              ...
              <input 
                ...
                className="w-full pl-9 pr-4 py-2 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-600 rounded-md text-sm focus:ring-1 focus:ring-indigo-500 outline-none text-slate-900 dark:text-white"
              />
            </div>
            <div className="relative w-48">
              <select className="w-full pl-3 pr-8 py-2 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-600 rounded-md text-sm focus:ring-1 focus:ring-indigo-500 outline-none text-slate-700 dark:text-slate-300 appearance-none">
          ...
          {campaigns.length === 0 ? (
            <div className="flex flex-col items-center justify-center max-w-md mx-auto text-center py-12">
              <h2 className="text-xl font-bold text-slate-900 dark:text-white mb-2">You have not created any email campaigns</h2>
              <p className="text-slate-500 dark:text-slate-400 mb-6 text-sm">
                Click on "Create campaign" and start designing your first email campaign.
              </p>
              <button 
                onClick={() => setIsNamePopupOpen(true)}
                className="px-6 py-2 bg-white dark:bg-slate-700 border border-slate-300 dark:border-slate-600 text-slate-700 dark:text-slate-200 rounded-full text-sm font-medium hover:bg-slate-50 dark:hover:bg-slate-600 transition-colors"
              >
                Create campaign
              </button>
            </div>
                  ...
                  {campaigns.map((campaign) => (
                    <tr key={campaign.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                      <td className="px-6 py-4">
                        <Link to={`/marketing/logs?campaignId=${campaign.id}`} className="font-medium text-indigo-600 dark:text-indigo-400 hover:underline">{campaign.name}</Link>
                        <div className="text-slate-500 dark:text-slate-400 text-xs">{campaign.subject}</div>
                      </td>
                      <td className="px-6 py-4">
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                          campaign.status === 'Sent' ? 'bg-emerald-100 text-emerald-800 dark:bg-emerald-500/20 dark:text-emerald-400' :
                          campaign.status === 'Scheduled' ? 'bg-blue-100 text-blue-800 dark:bg-blue-500/20 dark:text-blue-400' :
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-800 rounded-lg shadow-xl w-full max-w-md p-6">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-bold text-slate-900 dark:text-white">Add Campaign Name</h3>
            ...
            <input 
              ...
              className="w-full px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-md bg-white dark:bg-slate-900 text-slate-900 dark:text-white focus:ring-2 focus:ring-indigo-500 outline-none mb-4"
            />
            <div className="flex justify-end gap-2">
              <button onClick={() => setIsNamePopupOpen(false)} className="px-4 py-2 text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-md">Cancel</button>
      ...
      {isTypePopupOpen && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-800 rounded-xl shadow-2xl w-full max-w-3xl p-8">
            <div className="flex justify-between items-center mb-8">
              <h2 className="text-2xl font-bold text-slate-900 dark:text-white">Create a campaign</h2>
            ...
            <div className="mb-6">
              <h3 className="text-lg font-semibold text-slate-900 dark:text-white mb-2">Standard</h3>
              <p className="text-slate-500 dark:text-slate-400">Create a one-off campaign from scratch.</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <button 
                ...
                className="flex flex-col items-center justify-center p-6 border border-slate-200 dark:border-slate-700 rounded-xl hover:border-indigo-500 hover:shadow-md transition-all group bg-white dark:bg-slate-800"
              >
                <div className="w-24 h-24 bg-emerald-50 dark:bg-emerald-900/20 rounded-lg flex items-center justify-center mb-4 group-hover:scale-105 transition-transform">
              ...
              {/* SMS Option */}
              <button 
                className="flex flex-col items-center justify-center p-6 border border-slate-200 dark:border-slate-700 rounded-xl hover:border-indigo-500 hover:shadow-md transition-all group bg-white dark:bg-slate-800 relative opacity-70 cursor-not-allowed"
              >
                <div className="w-24 h-24 bg-indigo-50 dark:bg-indigo-900/20 rounded-lg flex items-center justify-center mb-4">
                  <svg width="64" height="64" viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg">
              ...
              {/* WhatsApp Option */}
              <button 
                className="flex flex-col items-center justify-center p-6 border border-slate-200 dark:border-slate-700 rounded-xl hover:border-indigo-500 hover:shadow-md transition-all group bg-white dark:bg-slate-800 opacity-70 cursor-not-allowed"
              >
                <div className="w-24 h-24 bg-green-50 dark:bg-green-900/20 rounded-lg flex items-center justify-center mb-4">
                  <svg width="64" height="64" viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg">
              ...
              <button 
                className="flex flex-col items-center justify-center p-6 border border-slate-200 dark:border-slate-700 rounded-xl hover:border-indigo-500 hover:shadow-md transition-all group bg-white dark:bg-slate-800 relative opacity-70 cursor-not-allowed"
              >
                <div className="w-24 h-24 bg-teal-50 dark:bg-teal-900/20 rounded-lg flex items-center justify-center mb-4">
                  <svg width="64" height="64" viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <span className="font-semibold text-slate-900 dark:text-white">Push</span>
