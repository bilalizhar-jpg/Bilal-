  ...
  return (
    <AdminLayout>
      <div className="space-y-6">
        ...
        <div className={`rounded-xl border ${isDark ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'} shadow-sm p-4 flex items-center justify-between`}>
          <div className="flex items-center gap-2">
            <div className="relative">
              ...
              <input 
                type="text" 
                placeholder="Search..." 
                className={`pl-10 pr-4 py-2 border rounded-lg text-sm outline-none focus:ring-2 focus:ring-indigo-500 ${isDark ? 'bg-slate-800 border-slate-700 text-white' : 'bg-white border-slate-200'}`}
              />
            </div>
            ...
            <button className={`px-4 py-2 rounded-lg text-sm font-bold flex items-center gap-2 border ${isDark ? 'border-slate-700 text-slate-300 hover:bg-slate-800' : 'border-slate-200 text-slate-600 hover:bg-slate-50'}`}>
          ...
          <div className="flex items-center gap-2 text-sm text-slate-500">
            <span>Per Page:</span>
            <select className={`border rounded-lg px-2 py-1 outline-none ${isDark ? 'bg-slate-800 border-slate-700 text-white' : 'bg-white border-slate-200'}`}>
              <option>10</option>
              <option>25</option>
              <option>50</option>
            </select>
              ...
              {programs.map((program, idx) => (
                <tr key={program.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                  ...
                  <td className="px-4 py-3">
                    <span className={`px-2 py-1 rounded-full text-xs font-bold ${program.status === 'Active' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400' : 'bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-400'}`}>
                      {program.status}
                    </span>
                  </td>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex gap-2">
                      <button className="p-1 text-indigo-600 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 rounded"><Edit3 className="w-4 h-4" /></button>
                      <button className="p-1 text-red-600 hover:bg-red-50 dark:hover:bg-red-900/30 rounded"><Trash2 className="w-4 h-4" /></button>
                    </div>
                  </td>
                </tr>
              ))}
